...
AWR-237: ships a make_stick-built libhidapi.dylib under _MEIPASS/lib/ so frozen
--run-streamdeck works on guest Macs with no Homebrew. The dylib is built from
the hash-locked hidapi 0.15.0 SOURCE at MACOSX_DEPLOYMENT_TARGET=12.3 (see
packaging/libhidapi_arm64.lock); Homebrew host dylibs are the wrong-floor trap
and are never candidates. Missing dylib fails closed here and in make_stick.
"""
import glob
import os
import sys
...
# make_stick ensure_hidapi_dylib writes here; RBSS_HIDAPI_DYLIB overrides.
_HIDAPI_BUILT = os.path.join(
    REPO_ROOT, ".build-wheelhouse-macos12-arm64-cp313", "libhidapi.dylib"
)


def _resolve_hidapi_dylib() -> str:
    """Path to the floor-gated built hidapi dylib; fail closed if absent.

    Authenticity is the locked SOURCE tarball + make_stick Mach-O gate, not a
    binary digest (built dylibs are not hash-stable). Never fall back to
    Homebrew paths — those inherit the host macOS floor.
    """
    override = (os.environ.get("RBSS_HIDAPI_DYLIB") or "").strip()
    candidate = override or _HIDAPI_BUILT
    if not os.path.isfile(candidate):
        raise SystemExit(
            "rbss_launcher.spec: libhidapi.dylib not found at %s "
            "(run make_stick's ensure_hidapi_dylib, or set "
            "RBSS_HIDAPI_DYLIB=/path/to/libhidapi.dylib). Refusing a "
            "Stream-Deck-dead guest bundle." % candidate
        )
