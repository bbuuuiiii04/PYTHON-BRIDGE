...
start_bridge() {
    ...
    STARTED_AT=$(date +%s)
    WARNED_MULTIPLE=0
    log_watcher "started bridge pid=$BRIDGE_PID follow=on anlz_direct=on pos_chain_direct=on master_seed_direct=on play_direct=on master_phrase_arm=default manual=${MANUAL_MODE}"
}
