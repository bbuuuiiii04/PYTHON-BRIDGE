...
- AWR-265 (2026-07-16): Color-clone collapse Steps 0–2 (master-plan R10). Mapping+A/B harness (`tools/awr265_color_clone_collapse.py`, `tests/test_awr265_color_clone_collapse.py`): 18 CONFIG_EDIT / 7 IMPOSSIBLE residuals / 4 DIY out-of-scope. Base palette-fed cues (`rt_drop_strobe`, `rt_drop_chase`, …) + weight-0 family palettes; default banks point at bases; clones (+7 strobes) retained in `legacy_color_suffix` with pad Legacy tab. Step 3 deletion gated: `tools/awr265_step3_delete_clones.py`. Config-only (no `led_color_engine.py` edits). Loader gate green on example+live. Bridge was stopped; no restart by implementer.
