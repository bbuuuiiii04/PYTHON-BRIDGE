...
validation_scope: software-only; existing LED/Govee behavior plus the 2026-07-13 split-local-UUID classifier correction reverified: local Rekordbox 7 loads regain the early phrase worker, real `/Volumes` loads keep the resolved-time-only rule; AWR-241 Template Lab beat meter status field documented; AWR-242 Template Lab UX (`target_role` + lab-route UI) documented; AWR-243 Template Lab functional fixes (live test_palette for lab play + ownership payload on request errors) documented; AWR-245 Template Lab Strip|Room preview hookup (pad serves sim view + profile read-only; fail-soft; preview-only) documented; AWR-247 Template Lab preview length (default full cue_beats) documented; AWR-248 Pad|Lab|Sim cross-nav documented; AWR-254 pad look-editor close dirty check documented; AWR-255 pad+lab stale-config banner (config mtime vs bridge process start; no bridge runtime change) documented; AWR-256 remnants ember_hold/decay wired (dim_beats removed as dead); AWR-258 data-integrity (lab lock+CAS, pad locked_palette, sim stale-mtime/bak/profile_error block); AWR-259 pad integrity tail (stale_look CAS, Discard-all copy, persisted last_applied, EDITOR_FIELDS); AWR-271 R9a Lab-in-Pad shell mount documented; AWR-272 R9b Save-draft/Accept verb pair documented; AWR-275 pad/lab playback follows the live music's BPM from a fresh status heartbeat (pad-process `SyntheticClock`/`CueTimer` tempo-follow via the existing ownership poll; beat-continuous re-anchor + 0.3-BPM wobble debounce; stale/absent falls back to the manual Preview tempo; no bridge runtime change) documented (details in `docs/guides/led_pad.md`); AWR-277 accepted Template Lab cues are first-class in the pad (named "Lab cue", Play enabled, `_play_spec` routes `lab:<draft>` through the pad `LabRenderer` as `lab_<draft>`) and the Template Lab Timing dropdown is demoted to a read-only design note (stored `timing_mode` preserved) documented; AWR-278 post-Accept lifecycle reconciliation + audit fixes (Reject un-wires an accepted look unless pad-edited/shared; Delete flips the draft to iterating; lab-cue titles follow the look name and rename drops the stale live look; re-Accept updates in place; Test-Palette keeps live lab tweaks; the shared tempo box is view-aware; cue length clamps to >=1; duplicate/rename check res.ok; bank row hides in Lab view) documented (details in `docs/guides/led_pad.md`); no LED rendering/output hardware validation
---
...
- AWR-274 (2026-07-16): R11 Setup demotion + Diagnostics corner (I4/J5 / D2). Sim Layout+Calibrate behind one **Setup** entry; gamma/gains behind closed Advanced with plain caution; FPS/provenance/frame/health telemetry in collapsed **Diagnostics**. Lab health strip under Diagnostics; beat meter stays. Cold-screen banned-string gate + calibrate gamma round-trip in `tests.test_led_ui_integrity.Awr274SetupDiagnosticsTests`. AWR-266 presentation + R6 gestures unchanged. Bridge runtime untouched.
- AWR-277 (2026-07-16): accepted Template Lab cues are first-class in the pad + honest timing note (two operator bugs). **BUG 1** — an accepted lab look stores `scene_ref = "lab:<draft>"`; the pad branded it "☁ cloud scene (not previewable)" and disabled its Play tile, and `LedPadService._play_spec` raised `cloud scene - not previewable in the pad` (the deeper bug — the AWR-260 adapter only lights `lab:` inside the frame-engine child; the pad process renders through `LabRenderer`, which lights `lab_<draft>`). Fix: `_play_spec` routes `is_lab_production_scene` looks through the new `_lab_production_play_spec`, resolving the draft, injecting engine colors kind-aware (`force_slot` for slot cues), and handing the renderer the `lab_<draft>` name it already draws; `pad-ui.js` names them "Lab cue" (badge + editor title + renderer-select), enables Play (`playable = realtime || labScene`), and shows a read-only param list pointing edits at the Lab view (`renderLabCueControls`) — no new edit pipeline. **BUG 2** — the Template Lab **Timing** dropdown (`timing_mode`) was styled as a functional playback control but nothing reads it for playback. Demoted to a read-only design note (`.lab-timing-note`; "A note on how this cue was designed. It doesn't change how the cue plays."); the stored field is preserved through save/stash (`currentPayload`/`stashEditor` carry `state.current.timing_mode`); the BPM-scope hint still reads the note honestly. Covered by `tests.test_awr277_lab_cue_first_class` (play-by-name renders lit frames end-to-end; slot-color injection; UI-integrity string checks; timing-field persistence). LED-pad process only; no bridge runtime change; hardware-unvalidated.
