...
- **RW-1 Export from SS** (`tools/export_soundswitch_pack.py`, `scripts/bridge_menubar.py`): one explicit menubar click runs a full read-only decode/compile/verify in a subprocess and publishes to `~/Music/SoundSwitch/rbss_canonical_pack`. The replace path uses macOS `renamex_np(RENAME_SWAP)` when available; its move-aside fallback retains a recoverable backup, restores the old pack if the second rename fails, and deterministically recovers the newest orphan backup on the next publish. A cross-process lock and in-process click guard serialize publishers. The tracked example config names this canonical pack while remaining `enabled=false`, `dry_run=true`, and `output_backend=none`; `load_pack()` expands the `~` path.
- After disk publication, the menubar distinguishes saved from live. If the bridge is stopped or pack output is disabled, it sends no command and reports saved-not-live. Otherwise it appends only the existing `set_soundswitch_pack reload` command and accepts a fresh `soundswitch_pack.pack_sha12` match as the reload acknowledgement. It never sends enable/backend, starts or restarts the bridge, or opens a device.
