...
- If an ordinary exception escapes the 200 Hz drain/tick/snapshot iteration, `StateManager._run` submits a direct pack ZERO frame, logs a bounded error counter, skips that instant, and continues. This darkens only the pack lane for the bad tick; OS2L/laser-MIDI and LED/Govee lanes are separate and may hold their last output until their next successful decision.
