...
- **Menubar pack on/off + auto-switch** (`scripts/bridge_menubar.py`): separate from the export/reload action, a pack on/off menu item enqueues `{"cmd":"set_soundswitch_pack","action":"enable","enabled":<bool>}` (`:390`), and `_auto_set_soundswitch_pack()` (`:868`, called from `:799`) flips pack output by SoundSwitch connection state. There is **no implicit hot-enable**: enabling still requires a real `output_backend="pack"` + `dry_run=false` + Enttec port, so a dry-run/none/no-config bundle stays disabled.
