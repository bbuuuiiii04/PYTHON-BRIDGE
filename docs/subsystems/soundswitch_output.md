...
- **Static Override Press/Toggle interaction mode**: the bridge honors the SoundSwitch-saved interaction policy instead of operator config. `soundswitch_project_decoder.py:855-888` decodes the saved `PushButton+0xc1` byte (`mode_raw==1` ⇒ `toggle`, else `press`); `PackMidiBinding`/`PackStaticLook` carry `interaction_mode ∈ {press, toggle}` (`soundswitch_pack_models.py:250,260`, `soundswitch_pack_loader.py:53,283-296`); and `SoundSwitchMidiInputAdapter` latches toggle slots on note-on and ignores note-off for toggles while keeping press/momentary behavior (`soundswitch_midi_input.py:220-252`). An unknown saved mode fails closed to momentary. The canonical pack is published to and loaded from the repo-local ignored path `local/soundswitch/rbss_canonical_pack`.
