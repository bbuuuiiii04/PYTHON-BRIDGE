...
- **RW-1 Export from SS** (`tools/export_soundswitch_pack.py`, `scripts/bridge_menubar.py`): one explicit menubar click runs a full read-only decode/compile/verify in a subprocess and publishes to `~/Music/SoundSwitch/rbss_canonical_pack`. The replace path uses macOS `renamex_np(RENAME_SWAP)` when available; its move-aside fallback retains a recoverable backup, restores the old pack if the second rename fails, and deterministically recovers the newest real-directory orphan backup on the next publish. Non-directory backup entries are removed and can never become canonical. A cross-process lock and in-process click guard serialize publishers. The tracked example config names this canonical pack while remaining `enabled=false`, `dry_run=true`, and `output_backend=none`; `load_pack()` expands the `~` path.
