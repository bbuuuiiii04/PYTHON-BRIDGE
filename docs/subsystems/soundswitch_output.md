...
- The menubar reuses one row for the copied pack state and export progress/result. A stale status file renders `Pack: Unknown`; the export/reload action still uses only the existing conservative reload command and never enables output or changes backend.
