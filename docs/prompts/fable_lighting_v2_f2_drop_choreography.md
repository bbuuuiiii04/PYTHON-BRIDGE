...
**Known-stale / evidence-only:** any project memory, any doc without a current status header, and old prompts/plans. Verify against current code before relying on them.

**IMPLEMENTATION STATUS — the following LANDED after this prompt was first authored (all implemented + software-tested in the same 2026-07-07 session, each adversarially reviewed PASS; HARDWARE-UNVALIDATED; auto-sync committed them, so re-verify file:line at your HEAD). Do NOT re-derive these as unimplemented or re-open RC2 as a live flicker bug — treat them as the current baseline F2 extends:**
...
- **AWR-141 (RC2)** — realtime wrap-flicker fix (`beat_sync_engine.py` `TriggerClock.advance`, `WRAP_HOLD_BEATS = 0.5`). The RC2 flicker is **FIXED**, not open; F2 builds on the fixed continuous clock (see deliverable 5).
- **AWR-142 (RC5)** — smart-drop blackout transport + runway observability (`led_dispatch_policy._dispatch_led_smart_drop_blackout`): every pre-drop blackout now logs `transport=realtime|cloud` and `runway_beats` — useful data if F2 touches the pre-drop blackout.
- **AWR-143** — drop-presentation label-rearm leak fix (`state_manager._drop_presentation_tick`): a presentation impact now requires `sp_state.smart_drop_crossing`, so a 2nd-chorus label re-arm no longer re-enters/extends the presentation window. **F2's per-drop / per-section presentation logic sits on top of this gate.**
- **AWR-144 (RC4)** — LED solo pre-dark hold (`led_dispatch_policy._dispatch_led_automation`): the LED drop look is suppressed (current look held) when the room-split plan's pending verdict is `LASERS_ONLY`, so there is no LED drop-look flash before a laser-solo blackout. F2's within-drop laser choreography composes with this.
...
- **Delegate the grind, keep the judgment.** Hand multi-file symbol tracing, signal-shape verification, and corpus/log sweeps to the Claude session at **`tmux a -t claude`** (drive it non-interactively: `tmux send-keys -t claude '<task>' Enter`, poll with `tmux capture-pane -t claude -p`). **Prefer it over Agent-tool subagents** — `tmux a -t claude` is a separate Max-x20 account, whereas Agent-tool subagents (including the read-only `bridge-triage` agent) bill the SAME rate-limited account you run on, so spend those sparingly and only when the tmux session is busy. `bridge-triage` still returns conclusions + `file:line` (never transcripts) when you do use it. You are the only Fable-tier agent: never spawn another Fable-tier subagent, and announce nested spawns. Keep the live-safety and design judgment on yourself; verify a delegated agent's load-bearing claims before relying on them.
