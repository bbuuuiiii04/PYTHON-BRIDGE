...
- **A regression may have a cheap first win.** Since this broke around 2026-07-07, an early phase can be as small as reverting or correcting the specific change you identify. If that alone restores Pad-smoothness and reliable blackout, say so plainly — the larger redesign then becomes an improvement Brandon *chooses*, not a fix he *needs*. A live-revert toggle back to the old cloud path is acceptable to him but not a priority; don't let a backwards-compat shim compromise the clean design.
