...
- **This is a REGRESSION, not a never-worked problem.** Brandon says the bridge **used to drive these realtime looks as smoothly as the LED Pad**, and it broke recently. A large batch of LED changes landed across **2026-07-07** (git history of the LED files below; the subsystem card's `AWR-141` / `AWR-142` / `AWR-144`, F2 drop choreography, intra-section look rotation, and the hold/idle fixes all date to that day). That batch is the prime suspect. **Your fastest route to root cause is what changed, not only how the code reads today.**
...
**Start from what changed.** Because this regressed around 2026-07-07, your first move is to walk the recent git history of the LED files (`git log -p --since='2026-07-05' -- govee_realtime_runner.py govee_realtime_transport.py govee_frame_renderer.py led_dispatch_policy.py led_dispatch_coordinator.py govee_owner_state.py beat_sync_engine.py state_manager.py`) and correlate suspicious changes with yesterday's log lines, rather than reasoning only from how the code reads today. The commit messages are auto-sync noise, so read the diffs, not the subjects. A regression usually has a specific culprit change; name it if you can find it.
