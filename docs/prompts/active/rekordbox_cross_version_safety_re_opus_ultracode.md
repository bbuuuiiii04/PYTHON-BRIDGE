...
- **Agentic driver / workhorse — your default for the tool-use-heavy loop** (Cursor Grok 4.5): Cursor bills it as their "most intelligent model," built for "difficult, long-running tasks that require creatively using tools to solve problems" — which is exactly this RE + orchestration job. Priced **$2/M input, $6/M output (base)** — strong value; use the **base**, not the **Fast** variant (**$4/M in, $18/M out** ≈ 3× the output cost), unless latency actually blocks you. **Ignore its CursorBench ranking** — Cursor disclosed an older snapshot of their own codebase leaked into its training, so that number is inflated; judge it by the task in front of you, not the benchmark. Use it to drive the per-function decompile loop, per-version verification, routine review rounds, and reading decompiler output. GPT 5.6 TERRA is an alternate at this tier. (Context window / effort levels / latency were **not stated** in Cursor's post — verify in the model picker rather than assuming.)
- **Premium — reserve for the hardest reasoning AND every SAFETY adjudication** (Opus 4.8, Fable 5, GPT 5.6 SOL): the RE call that decides the version-extension mechanism, and — per the hard exception below — every judgment about whether a clamp bounds every tempo path / the reader fails closed / a strobe surface is covered. High/xhigh effort **only where it changes the answer** (max rarely — it overthinks with diminishing returns). Do not delegate a safety adjudication to Grok 4.5 or the cheap tier no matter how much cheaper.
- **Cheap / fast — bulk mechanical work** (Composer 2.5, still offered): running `otool`/`strings`/`nm`, collecting symbols, diffing tables, boilerplate tests, doc edits. Low effort.
