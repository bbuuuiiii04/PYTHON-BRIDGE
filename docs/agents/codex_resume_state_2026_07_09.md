...
| AWR-184 parked finding (NEXT SESSION FIRST ITEM) | ledtune post-gate: rung 0b precedes the stop check and reads no full-band audibility → vocal-stop class could get blackout-4 instead of the calibrated 8 (taste-length only, both blackouts). Executive existence sweep: class is REAL — 55/716 v4 tracks (superset any-beat). Fix = one-line precedence guard (stop first, 0b yields) + per-drop confirmation + Utopia/Killa/Caramelle pins (Utopia not among hits — pins look safe) | docs/research/awr184_stop_ambiguity_sweep_2026_07_09.md |
