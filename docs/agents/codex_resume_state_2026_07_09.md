...
| **Undecided operator items** | ~~Pre-drop tease~~ **RULED DEAD 2026-07-09 ~21:5x (his words: "i will never use pre_drop" — the pre_drop look role stays empty forever; pre-drop DARKNESS/blackouts are a separate system and STAY)**; veto list (strobe_red_white side B, groove width 2.5, NEUTRAL→small laser silence, balloon gray-zone 0.30–0.35); drop-echo theme-and-variation (the one surviving creative proposal) | Surface once each when relevant; never re-litigate decided items |
