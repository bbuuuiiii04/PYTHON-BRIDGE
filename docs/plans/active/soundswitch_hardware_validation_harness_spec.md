...
- [confirmed] At this spec's verification point, `config/soundswitch_pack_player.json` was absent
  and the exact bridge-process check returned `0`. No process, config, output, or device state was
  changed.
- [confirmed] The repo has no repeatable SoundSwitch hardware record. HW-001 is `not logged` and
  ROAD-003 still asks for repeatable logs (`docs/status/active_work_registry.md:43-46`,
  `docs/status/active_work_registry.md:60-66`). The current hardware log has only a generic entry
...
5. **Post-start gate.** Run the exact single-process check below. Require `1`, a fresh status file,
   ...
   overlays remain idle. More than one process or any unexpected light is an immediate physical-kill
   and stop condition.
...
7. **Emergency rehearsal.** With a deliberately low-risk non-zero look active, the operator uses
   ...
   the bridge gracefully so the owned sender attempts its zero. Restore the physical path only after
   a separate operator approval. Do not use the menubar laser emergency item as proof of pack-DMX
   emergency masking: the pack driver currently passes `emergency=False`
   (`state_manager.py:3339`).
...
The procedure must explain that the broad pattern is the repo's accepted operator check
(`CLAUDE.md:25-27`) and must be run before and after any approved restart. If it returns anything
other than `1` after start, do not proceed.
...
2. All file/line claims were rechecked at `ab4d293`.
...
9. Adversarial case: a non-zero Enttec frame survives `kill -9`; the physical kill plus graceful-zero
   restore ordering prevents the procedure from treating process death as safe.
