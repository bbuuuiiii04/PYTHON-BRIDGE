...
| Template Lab Round 3 (`docs/plans/active/template_lab_round3_codex_spec.md`, AWR-126) — housekeeping, `61304ad` | A default-hidden rejected-drafts filter chip (`Rejected (n)`) in the `/lab` drafts list — pure UI; `GET /api/lab/list` keeps returning every entry regardless of status. `LabRegistry.delete()` + `POST /api/lab/delete` remove a draft's `drafts.json` entry only, refusing with `{"ok": false, "error": "stop_playback_first"}` while that exact draft is the one currently playing, and never touching the corresponding function in `effects_lab.py`. | implemented/software-tested; hardware-unvalidated |
