# Spectral program operating model v7 (2026-08-02)

**Status:** design proposal — `planned`, not adopted, nothing implemented. This revision answers
`ROUND_6_fable.md` and supersedes
`docs/plans/active/spectral_program_operating_model_v6_2026_08_02.md`. It does not authorize code,
...
- an unchecked development sheet requires `WAITING_FOR_CHECKER`, the stored checker-wait start,
  the §8 recorded failed checker-acquisition attempt, and the mandatory unchecked line. Its
...
The sender accepts that state only when the run record also contains a checker-acquisition
attempt, made at or after the run entered `WAITING_FOR_CHECKER`, whose recorded outcome is
failure — no eligible provider family was available or accepted the job. Each unchecked send
requires its own fresh recorded failed attempt, so "unavailable" is a stored fact, not a seat's
claim. The run stays `WAITING_FOR_CHECKER`, its delivery is stored, and the §4 proof, §3 printer,
run record, and hash-bound sender all still applied to it.

[assumed] When an eligible checker later becomes available, runs whose sheets were delivered
unchecked are checked oldest first, before that checker accepts any new run from the same builder.
Nothing can force a check while no checker exists anywhere; that boundary stays visible because
each further unchecked sheet carries a since-time that keeps growing older.
...
1. **One governed laser development slice.** Build `develop --lane laser` around the current
   ...
   hash, a lane refusal placed inside a full-list body, no checker, an unchecked sheet missing its
   unchecked line, an unchecked sheet accepted from `WAITING_FOR_CHECKER`, an unchecked send
   attempted without a recorded failed checker-acquisition attempt, a full list or evaluation
   attempted from that state, delivery without a sealed run id, `CHECK_FAILED` after an unchecked
   delivery, a failed correction delivery, and a newer result attempted before correction. Do not
   claim the operator surface is governed until the sender is active.
...
the checker would have caught. The mandatory unchecked line, the recorded failed
checker-acquisition attempt, the stored delivery, oldest-first checking when a checker returns,
and the sender's automatic correction block on a later `CHECK_FAILED` make that visible and bound
its abuse; they do not prevent the original error, and nothing can force the deferred check to run
while no checker exists anywhere. Total chat loss delays both the correction and every newer
governed result.
...
checker is unavailable, a one-lane development sheet can still reach you, but only after a real
failed attempt to get a checker is on record, it always says it is unchecked, and a later failed
check is corrected by name.
