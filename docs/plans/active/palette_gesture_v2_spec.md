...
## Part A — Context (verified at `fd86318`; read, do not implement)

- [confirmed] The deck already sends BOTH edges: `key_to_message(key, pressed)`
  emits note_on at press and note_off at release
  (`streamdeck/streamdeck_midi.py:130-137`). No deck-side change is needed to
...
- [confirmed] Gesture v1 lives in `led_palette_control.py::_handle_palette`
  (snapshot `queued_palette == name` → override, else queue) and
  `_handle_lock` (toggle); events carry no phase. The engine has
  `queue_palette` (validated store), `override_palette(name, start_beat,
  end_beat)` (fade + `_hold_track` + queue-consume), `lock()`/`unlock()` —
  and NO unqueue method (`led_color_engine.py`).
- [confirmed] Lock pad plumbing to retire from the surface: config
  ...
  (`streamdeck/streamdeck_midi.py:181-191` region). The runtime commands
  `led_palette_lock` / `led_palette_unlock` are a separate debug surface and
  STAY.
- [confirmed] The idle-swatch illegibility root cause: inactive pads render
  `max(12, int(part * 0.22))` per RGB channel
  (`streamdeck/streamdeck_midi.py:299-300`) — linear 22% crushes hue identity
  on the small pads. The feedback file's per-palette `rgb` values are correct
...
2. On `LED_PALETTE_PAD`:
   - Runtime-command events (`intent` present, no `phase`) keep today's
   ...
   - `phase == "up"`: `held = now - _pad_down.pop(name, now)` (missing down →
     0.0 → tap). While Rainbow mode is on: drop both phases (pads inert).
     - `held >= long_press_s` → **take-and-hold**: if engine snapshot shows
       `lock` and `current_palette == name` → no-op (idempotent re-hold);
       else `engine.override_palette(name, …existing anchor derivation…)`
       then `engine.lock()` (rule 7: the mid-fade lock pins the fade target
       on completion — reuses the existing rule-6/9 behavior, no new fade
       code).
