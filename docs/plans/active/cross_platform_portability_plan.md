...
> **Operator decision (2026-07-04, recorded at parking): in USB mode the bridge drives lighting via its native pack player (AWR-107), not SoundSwitch on the host.** Re-scope on pickup: the §1.3 virtual-MIDI "hard Windows blocker" shrinks to Stream Deck + bridge-internal look wiring (no bridge→SS look-selection channel on a foreign host); §4's boundary-honesty flips for pack mode — the bridge renders the DMX frames itself, so authored strobe content *is* clampable at the frame sender (SS-app mode keeps the old boundary); Phase 3's verify targets pack render + Enttec output instead of "SoundSwitch rotation"; and the Enttec/pyserial path (an undeclared dependency — AWR-124 R5) becomes the primary output in every packaged-build manifest. AWR-124's R8/E2/stitch-3 re-scope with it.
