# Spectral program operating model v6 (2026-08-02)

**Status:** design proposal — `planned`, not adopted, nothing implemented. This revision answers
`ROUND_5_sol.md` and supersedes
`docs/plans/active/spectral_program_operating_model_v5_2026_08_02.md`. It does not authorize code,
...
- an unchecked development sheet requires `WAITING_FOR_CHECKER`, the stored checker-wait start,
  and the mandatory unchecked line. Its delivery is stored, but the run remains
  `WAITING_FOR_CHECKER`.
...
The sender accepts that exact state under §2, the run stays `WAITING_FOR_CHECKER`, its delivery is
stored, and the §4 proof, §3 printer, run record, and hash-bound sender all still applied to it.
...
A worker can still choose a convenient initial panel or a weak but runnable falsifier. The report
names the panel chooser and every track, emits no positive verdict, and the budget limits the cost;
none of that makes the choice representative or wise.
...
1. **One governed laser development slice.** Build `develop --lane laser` around the current
   ...
   unchecked line, an unchecked sheet accepted from `WAITING_FOR_CHECKER`, a full list or evaluation
...
the checker would have caught. The mandatory unchecked line, the stored delivery, and the sender's
automatic correction block on a later `CHECK_FAILED` make that visible; they do not prevent the
original error. Total chat loss delays both the correction and every newer governed result.
...
panel can still be convenient and every report says who chose it; the evaluator never calls a
change progress or improvement. If the independent checker is unavailable, a one-lane development
sheet can still reach you, but it always says it is unchecked, and a later failed check is corrected
by name.
