...
everywhere. The store schema bump to 2 makes E2/E3 refuse every v1 store by the
existing refusal-by-construction law, so no mixed-version grade can ever exist.
The operator's trust dossier is regenerated AFTER the rebuild — that
regeneration is the final deliverable of the IMPLEMENTATION, not of this spec.
...
- The product law `library_scaled = within_track × track_weight` is **not** the
  defect. It collapses (`rho` +0.980 with track weight, +0.171 with the moment)
  only because `within_track` varies 1.2× while track weight varies 3.8×. Fixing
  D1 and D2 restores it without changing the formula.
...
Measured: today's formulation reads **−0.675** (fails); the v2 formulation reads
**−0.480** (passes). `[assumed-reasonable]`, pinned, moved only by spec amendment.
...
**1c. Extend the acceptance verdict.** New signature, precedence
corpus size → loudness proxy → **compression proxy** → degenerate → ok:
...
with the new branch returning `(False, "compression_proxy")` when `rho_drama is
None or abs(rho_drama) > DRAMA_ACCEPT_MAX`. `None` fails closed, exactly as the
existing `rho is None` branch does (`:199-200`).

**1d. Bump the store schema.** Add `STORE_SCHEMA_VERSION = 2` to this module as
the single source of truth and have both the report tool and `section_energy_v0`
reference it conceptually (they may not import each other — restate the literal
`2` in `section_energy_v0.py` with a comment naming this spec, matching the
existing deliberate no-import convention at `section_energy_v0.py:39`).
...
550 = 2.2%**). The tool already calls `read_anlz_drops` (`:98`) and therefore
already holds `data.drop_beat_indices` for free.
...
**2c.** Store shape v2 (`build_store`, `:108-136`):
...
Measured effect on the real corpus: sections railed **66.75% → 5.28%**; `chorus`
sections pinned at 1.000 **83.9% → 0.5%**; tracks whose ≥3 chorus sections can be
ranked **0.623 → 1.000**; chorus-vs-breakdown separation 0.836 → **0.501**. The
...
a comment naming AWR-291. This is what makes v1 stores refuse by construction.
...
Rationale for each, measured on the real corpus (today's value → v2 value):

- **`SATURATION_GATE_MAX = 0.20`** — 0.6675 → **0.0528**. Today fails by 3.3×;
  v2 passes with ~4× headroom. This is the gate that would have caught D2 on day
  one, and it is the diagnostic the ceiling-effect literature prescribes.
- **`RANKABILITY_GATE = 0.90`** — 0.623 → **1.000**. Measured as: among by_genre
  ...
  to the cue-casting consumer whatever else it does.
- **`SEPARATION_GATE = 0.25`** — 0.836 → **0.501**. A direction-of-truth floor,
...
**4c. Record the deferred follow-up in the report header** (see Task 10 item 4).
...
Why these three and not others: measured within-track drop-discrimination ratio
(median within-track spread ÷ corpus IQR — above 1.0 means the signal varies as
much between one track's own drops as across the whole library), and independence
from the term that today's grade already is:

| series | discrimination ratio | rho vs onset density |
|---|---|---|
| `perc_high` | **1.26** (best of all 21 cached series) | +0.273 |
| `onset_density_midhigh` | 1.10 | — |
| `growl_flatness` | 0.97 | +0.066 |
| `sub_db` (dropped) | 0.84 | — |
| `full_db` (dropped) | **0.75** (weakest) | — |
...
matters: its within-track discrimination ratio is **1.00 vs onset's 1.10**; it
carries the two highest correlations of any candidate pair (**+0.248** with
`onset`, **+0.268** with `perc_high`); and adding it as a fifth term would
...
LEVEL_SEPARATION_DB = 3.0       # G6 (replaces the old cross-scale separation)
MIN_ACCEPT_N = 100              # unchanged
```
...
**G6 replaces the old G3 and fixes a real measurement error.** The old gate
compared E3's three-term grade against E2's one-term section grade — two
...
asserts direction of truth **in dB**, immune to whatever normalisation either
grade uses: `median(drop-window mean rel_dB) - median('low' section mean rel_dB)
>= 3.0`. Measured section levels by label — `chorus` p50 −1.17, `up` p50 −5.98,
`low` p50 −7.18, drop windows p50 −0.67 dB — give **+6.51 dB**, a 2.2× margin. If
the window math ever points the wrong way this goes negative and nothing else
would catch it.

New verdict, precedence corpus floor → coverage → term saturation → term
correlation → composite IQR → rankability → level separation:
...
def gates_verdict(n_by_genre_eligible: int, n_graded: int,
                  ...
                  iqr: "Optional[float]",
                  rankable_fraction: "Optional[float]",
                  level_separation_db: "Optional[float]") -> "tuple[bool, str]":
```

reasons: `"insufficient_corpus"`, `"insufficient_coverage"`,
`"saturated_term"`, `"redundant_terms"`, `"degenerate_distribution"`,
`"unrankable_grades"`, `"inverted_or_flat_separation"`, `"ok"`. Every `None`
fails closed.
...
the rankability fraction (denominator: tracks with ≥3 graded drops), and the G6
level separation in dB. Also print the **count and percentage of drops graded on
the `"corpus"` body basis** — that number is how you would notice the fallback
firing more than the measured 2.2%.

Feed all of it to the new `gates_verdict`. Keep G1, the MUST-print
...
- `acceptance_verdict()` precedence table, exercising **every** branch in order:
  ...
  `degenerate_component`, `ok` — including `rho_drama=None` failing closed.
...
- `load_track_weight_store` refuses a `schema_version: 1` store (the v2 refusal
  law) and still refuses `accepted: false`, malformed, and missing.
...
`forbidden_assumptions` (`:775-777`) with v2 text stating: the v2 component set
(`body_duty`, `brightness_med`, `onset_mh_mean`, `growl_flatness_mean`); the two
acceptance controls (loudness |rho| ≤ 0.50 **and** dynamic-range |rho| ≤ 0.55);
store `schema_version: 2` with `drop_body_distribution`, which makes v1 stores
refuse by construction; E2's span `-12..0` with the saturation / rankability /
separation gates; E3's four required terms with the rank-based body, its
`body_basis` field, and its six gates; that all four E3 terms are required so no
grade is ever a re-weighted mean; and that the flags, threads, fences, status
surfaces and flag-off byte-identity kill tests are **unchanged**.
