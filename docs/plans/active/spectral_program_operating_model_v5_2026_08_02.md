# Spectral program operating model v5 (2026-08-02)

**Status:** design proposal — `planned`, not adopted, nothing implemented. This revision answers
`ROUND_4_fable.md` and supersedes
`docs/plans/active/spectral_program_operating_model_v4_2026_08_02.md`. It does not authorize code,
...
- **`READY`** — all four lanes finished, every output rule passed, a complete run record was
  published, and an independent checker approved the exact list. The full list is pasted in chat.
...
| `WAITING_FOR_CHECKER` | "The run finished, but an independent check has not been available since <time>." |
...
[assumed] The sanctioned sender accepts exactly three governed body types — a full list, a
development sheet, and an evaluation report — and each only with a published run id and a body
whose hash matches that run's stored output. Anything without a run id, with a wrong hash, or with
a missing or wrong mandatory first line for its type is rejected. It rejects side-script output. A
seat can still paste arbitrary text into chat; that is an accepted limit, not a solved enforcement
problem.
...
[assumed] An evaluation report renders under its mandatory §1 evidence-scope first line, and when
its run was shown before independent checking under §8, the unchecked line directly below it.
...
[assumed] There is no `score` command and no combined ordering. The command is:
...
and its chat body reaches the operator only through the §2 sender, bound to its published run id
and body hash, under its §1 evidence-scope first line. An invented number or comparison pasted in
...
refused track returns `NOT_COMPARABLE`; easy-track selection cannot become an improvement. With no
such preregistered panel, `evaluate` never emits an improvement or regression sentence: it prints
each run as a single-run description labelled `NO PREREGISTERED PANEL — differences not assessed`.
Panel membership is still a human choice; the panel's tracks print in the report, so a
convenient panel is at least visible, not certified good.
...
- whether the body was shown unchecked before verification.
...
`UNCHECKED — independent check unavailable since <time>` directly under its development header.
The run stays `WAITING_FOR_CHECKER`, its record stores that it was shown unchecked, and the §4
proof, §3 printer, run record, and hash-bound sender all still applied to it. When a checker later
returns `CHECK_FAILED` on a shown sheet, a correction naming that exact sheet must be posted in
chat before any newer result. An unchecked sheet can still be wrong in ways the checker would have
caught; the label and the mandatory later correction are the visibility, not a prevention.

---

## 9. Work state survives a dead seat

[assumed] Stored states are:
...
[assumed] A worker cannot swap in easy tracks, omit refusals, or change evidence/substrate and call
the result comparable, because §6 refuses comparative output without a panel registered before the
candidate's builder work and checks that panel against both manifests. A worker can still choose a
weak but runnable falsifier; the budget limits the cost, and the weakness remains visible in the
experiment row.
...
[assumed] The sender requires the actual body, a published run id, a matching output hash, and the
correct mandatory first lines for its type: the full-list shape, the development header (plus the
unchecked line when §8 applies), or the evaluation evidence-scope line. It rejects path-only
replies, internal diagnostics, and any body type it does not recognize. It cannot prove free prose
is relevant or natural; that remains an accepted limit.
...
1. **One governed laser development slice.** Build `develop --lane laser` around the current
   ...
   development header, and the run-id/hash sender **as one switch**. Test missing phrase data, a
   buildup row, a row crossing section end, missing length, clock mismatch, wrong lane, wrong body
   hash, no checker, an unchecked sheet missing its unchecked line, and a full list attempted
   unchecked. Do not claim the operator surface is governed until the sender is active.
2. **Evaluator and preregistered comparison panel.** Build §6. Test partial-key unjudged output,
   ...
   or mismatched evidence-scope line, and a comparison attempted without a preregistered panel.
...
slot. If only one usable builder/provider slot remains, hearing work gets it and adoption pauses.
Before the experiment database exists this rule is enforced by someone remembering; after §5 it is
stored and the launcher refuses an adoption lease that would consume the reserved slot. The design
does not claim that ordering alone guarantees hearing progress.
...
the checker would have caught. The mandatory unchecked line, the stored shown-unchecked fact, and
the required correction on a later `CHECK_FAILED` make that visible; they do not prevent it.
...
membership choice, and slot choice still require judgment. Recording and exposing those choices
...
pinned before the work started, so easy-track swaps cannot pose as improvement. If the independent
checker is unavailable, a one-lane development sheet can still reach you, but it always says it is
unchecked, and a later failed check is corrected by name.

[assumed] Lowkey stays a known exposed regression track — every Lowkey result says so on its first
line. A future track you choose and hear cold is the only new evidence that the machine
