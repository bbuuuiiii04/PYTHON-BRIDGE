# Spectral program operating model v4 (2026-08-02)

**Status:** design proposal — `planned`, not adopted, nothing implemented. This revision answers
`ROUND_3_sol.md` and supersedes
`docs/plans/active/spectral_program_operating_model_v3_2026_08_02.md`. It does not authorize code,
...
report carries `EXPOSED COMPLETE REGRESSION — not a blind result`.
...
[assumed] The sanctioned sender accepts only a published run id and a body whose hash matches that
run's stored output. It rejects side-script output. A seat can still paste arbitrary text into chat;
that is an accepted limit, not a solved enforcement problem.
...
unnamed lane, or a row contradicts its evidence record. It never repairs, trims, or silently drops
an invalid row.
...
behind this wrapper may claim complete capture.
...
[assumed] An experiment pins its evaluation panel before builder work: ordered track identities,
audio hashes, selected lanes/classes, evidence snapshot, substrate hashes, and required refusal
rows. Baseline and candidate manifests must match that panel exactly. A missing, added, substituted,
or newly refused track returns `NOT_COMPARABLE`; easy-track selection cannot become an improvement.
Single-run description remains allowed but is labelled `NO BASELINE — differences not assessed`.
...
[assumed] Each experiment stores its question, allowed files, executable falsifier, fixed
...
the result comparable because §6 checks the pinned panel against both manifests. A worker can still
choose a weak but runnable falsifier; the budget limits the cost, and the weakness remains visible
in the experiment row.
...
[assumed] The sender requires the actual list body, published run id, matching output hash, and
correct full-list or development header. It rejects path-only replies and internal diagnostics. It
cannot prove free prose is relevant or natural; that remains an accepted limit.
...
1. **One governed laser development slice.** Build `develop --lane laser` around the current
   ...
   hash, and no checker. Do not claim the operator surface is governed until the sender is active.
2. **Evaluator and fixed comparison panel.** Build §6. Test partial-key unjudged output, exposed
   complete extras, one-to-one long rows, deterministic ties, duplicate rows, refusals, descriptions,
   changed track selection, changed audio identity, and the regression floor.
...
[assumed] A seat can hand-paste ungoverned output. Missing run id and headers are tells, not a
physical block on typing.
...
[assumed] Free prose, `human_only`/`not_applicable` classification, falsifier quality, and slot
choice still require judgment. Recording and exposing those choices does not make them correct.
...
classified not applicable. Comparisons use the same pinned tracks and inputs, so easy-track swaps
cannot pose as improvement.

[assumed] Lowkey stays a known exposed regression track. A future track you choose and hear cold is
the only new evidence that the machine generalized. Your ear remains the acceptance gate.
