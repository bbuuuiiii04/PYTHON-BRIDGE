...
| Menubar auto-switch | Implemented and software-tested. `_auto_set_soundswitch_pack()` (`scripts/bridge_menubar.py:883`) flips pack output by SoundSwitch connection using `set_soundswitch_pack action=enable`; a fresh disconnected `pack_start_failed` auto-enable gets one bounded retry. No manual pack button; no implicit hot-enable; enabling still requires a real `output_backend=pack` + `dry_run=false` + Enttec port. AWR-238: bridge-side boot `pack_start_failed` also retries via supervisor until Enttec/IAC appear; SS-MIDI `InvalidPortError` rebinds instead of worker death (software-tested; hot-plug live-unvalidated). |
...
| Audit P2 overlay diagnostics | Implemented and software-tested. While SoundSwitch-connected suppression keeps pack output at software ZERO, copied pack status exposes `overlay_suppressed` booleans for held static, blackout, and degraded input. |
...
- [x] Resolve the active `unverified_parity` blockers before trusted
  publication. Fresh export now reports active lanes `algorithm_generalized: 70`,
  `oracle_proven: 14`, `unverified_parity: 0` (re-baselined 2026-07-16, AWR-276); inactive unverified documents
