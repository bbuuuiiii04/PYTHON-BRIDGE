...
| Menubar auto-switch | Implemented and software-tested. `_auto_set_soundswitch_pack()` (`scripts/bridge_menubar.py:883`) flips pack output by SoundSwitch connection using `set_soundswitch_pack action=enable`; a fresh disconnected `pack_start_failed` auto-enable gets one bounded retry. No manual pack button; no implicit hot-enable; enabling still requires a real `output_backend=pack` + `dry_run=false` + Enttec port. AWR-238: bridge-side boot `pack_start_failed` also retries via supervisor until Enttec/IAC appear (software-tested; hot-plug live-unvalidated). |
