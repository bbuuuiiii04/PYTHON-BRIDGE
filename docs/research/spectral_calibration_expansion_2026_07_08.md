...
## 6f. Operator listening pass, round 5 (FINAL desk batch) — rainbow, balloon tiebreaker, 16-confirm (2026-07-08)

Verdicts (8 items; desk calibration CLOSED at 41 total verdicts over 5 rounds):

- **Rainbow class CONFIRMED at 4/5 precision on the profile query** (clean grit ≤0.04 + synth-washed ≥0.5 + bright 550-1500 Hz + punch ≥0.6): Adventure of a Lifetime (GECHA), Temperature (MERCO), crystallized, Die Young (RAFAEL) — all **"yes" full-speed rainbow**; Espresso (Andrew Marks) = "works, but more **off-white**" (→ the existing `white_sand` Warm Ivory palette identity, not rainbow). Combined with Clarity (weapon) and NO BROKE BOYS (maybe): **a per-track rainbow-identity classifier is derivable from already-cached measurements** — handed to the color lane.
- **Balloon-vs-blackout tiebreaker ANSWERED:** INNERBLOOM (BUNT) @2:59 (build perc 0.20) and Caramelle @0:44 (perc 0.10) = **balloon-shrink** (Caramelle "2-beat probably" — short black + shrink family); vs the 4-black busy labels (Diamond Therapy, Errday@1:31, Tremor@3:22 — percussive groove/big-room builds). Don't Call Me Baby @0:59 (perc 0.66 → "definitely balloon") **drops out of the set — operator: "not really a drop, I'll change that in the track"** (marker fix #6). Clean split on the remaining labels: **build-window `perc_full` low (≲0.3, melodic riser/swell) → balloon-shrink; high (≳0.4, drums driving) → 4-beat black.** Fittable shape for the F2 spec; pin the exact boundary there.
- **16-beat rule SHARPENED by a rejection:** Caramelle @2:07 — a textbook measured collapse (−14 dB, perc 0.16, 45-beat run) — gets "**balloon shrink or 2-beat**", NOT 16. With the three validated 16s (kidstopbreathing, Satisfaction, Age Of Love — all hard/dark monsters), the rule is now: **16 = true collapse + HARD/DARK monster incoming (WALL/COMET-grade); melodic/mainstage tracks never earn 16 regardless of collapse depth.** The incoming-drop character term dominates the depth term.
- **INNERBLOOM (BUNT): phrase markers literally inverted** (drop↔buildup; operator fixing — marker fix #7). Clarification recorded: phrase/drop markers come from Rekordbox's analysis, not this pipeline; v4 only reads audio around them.
- **Meta-finding:** the labeling process doubled as a marker-hygiene audit — 7 tracks' Rekordbox phrasing corrected across the 5 rounds. Marker quality is a live confound for every consumer; the F2 spec's marker-hygiene section (§6c item 4) is load-bearing.

**Final desk-calibration state:** darkness ladder pinned (1-2 groove/run-in · 4 default emphasis · 8 intense trap/stops · 16 collapse+hard-monster only · balloon-shrink for melodic swells, perc-split); perc-cut flick validated verbatim; family classifier sound (one trap miss); tier scorer needs redesign (6 documented failure cases, era/hardness cluster); rainbow + red-for-hard-techno + off-white labels handed to the color lane; wobble detector blind to slow wub (named false negative); grid-health flag recommended for variable-BPM tracks. **No further desk labeling — next verdicts come from the room once F2 is built.**
