...
AWR-281 (2026-07-17): the LED cue dedup (four slot comet-trains + both freestyle-nebula
aliases + legacy `sparkle` + v1 `drop_firework_explosion` deleted; remnants renamed to
`sparkle`, firework v2 renamed to `firework_burst`; live config 65->58 looks) is
software-tested only. The bridge was NOT started and no running process was touched — the
thinned banks and both renames take effect on the next operator bridge start. No Govee
packet, strip audition, or room-visible validation was performed. The kill list itself came
from the operator's own live audition on 2026-07-17; what remains unvalidated is how the
surviving drop/post-drop rotation reads in the room once the config loads.

AWR-256 (2026-07-15): the remnants tail (effect renamed `sparkle` 2026-07-17) ember_hold/decay
wiring is software-tested only. No bridge restart, Govee packet, strip audition, or room-visible
...
live-unvalidated — no hardware run recorded here.
...
| pending | LED cue dedup: comet-train kill + honest sparkle naming (AWR-281) | pending | `govee_frame_renderer.py`/`led_pad_controls.py` + both configs at HEAD a6ef4120+; bridge never started | audition the thinned banks on the next bridge start: drop = white_aggressive / center_burst / strobe / firework_explosion / palette_comet, post_drop = center_comet / firework_chase / sparkle / palette_comet. Confirm the repeats the operator rejected are gone, that the post-drop sparkle still reads as the tail he likes, and that no surviving look changed appearance (the renderer claims byte-identical frames). |
