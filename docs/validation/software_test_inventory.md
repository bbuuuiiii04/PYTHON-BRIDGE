...
| Laser | laser config/director/executor/MIDI dry-run tests | cannot prove physical safety. `tests/test_drop_lifecycle.py` covers true smart-drop crossings, the capped chorus-to-chorus second hit, post-cap demotion, per-section reset, single-marker behavior, and LED flat-window parity. Audit P4 adds send-error reopen recovery, bank-gate restore, config fallback/cooldown validation, deprecated `pre_drop_scene` tolerance, blackout-mask refcount, and Laser Pad live-toggle command append coverage. AWR-220 adds `drop_laser_qualifies` / `laser_tier_min` + ratio-cap planner cases in `tests/test_drop_presentation.py` and section-verdict latch cases in `tests/test_state_manager_drop_presentation.py` (true-drop only; coarse re-arm inherits leds_only; clear on track/deck/stop/scripted/seek-back). AWR-219 adds Enttec dead-handle reconnect, stop-during-reconnect race, and post-startup factory non-poisoning in `tests/test_enttec_dmx_pro.py` / `tests/test_soundswitch_frame_sender.py` | AWR-219/220 software-tested only; no real USB flap or room-visible laser selectivity proof |
