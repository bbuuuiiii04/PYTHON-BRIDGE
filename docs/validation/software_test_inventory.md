...
| Runtime commands | parser/handler/status writer tests plus menubar watcher-launch tests and watcher function tests. AWR-192 pins the slim menu inventory and four-row Status submenu. `tests/test_bridge_menubar.py` also pins the LEDs Status-row display-only path: realtime `last_error` (e.g. `transport_send_failed`) surfaces as degraded suffix `sends failing` under **On**, healthy empty `last_error` stays clean, and a cloud-adapter `degraded_reason` still wins over realtime. AWR-221/RB7216 pins the common **Patch Rekordbox** action, distinct checking/patching flags, source/frozen argv, cancel suppression, and watcher-thread GTA + deep/strict completion verification; **Checking Rekordbox…** and **Rekordbox Patched** are grey, while invalid/absent/unknown stays actionable. Rebuild-USB tests pin state text, click-time fingerprinting, complete build-input roots, and success-only stamping. | Menubar tests prove pure state, dispatch, and completion logic only. They do not prove rendered AppKit UI, live codesign/App Management, bridge/watcher health, a physical foreign-Mac attach, or hardware. AWR-222 remains live-unvalidated / unknown. |
