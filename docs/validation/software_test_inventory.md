...
| USB launcher (M1) | `tests/test_launch_profile.py` pins the single-source launch env — the 19 `RBSS_*` flags + the laser-config path that the watcher and the future bundle both read from `launch_profile.py`, including the AWR-149 deltas (`RBSS_LED_TRANSPORT_COOLDOWN=0`, `RBSS_LED_TRANSPORT_STICKY` absent) — and greps `scripts/ss_bridge_watcher.sh` to prove it now reads `launch_profile` instead of a hand-maintained flag list | pure-function + grep tests only; they prove the watcher and bundle share ONE env source, not that a PyInstaller bundle packages, runs, or reads Rekordbox memory identically to a source run (that is the operator-gated M1 parity run) |
