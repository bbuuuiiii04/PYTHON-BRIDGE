...
| Runtime commands | parser/handler/status writer tests plus menubar watcher-launch tests and watcher function tests (`tests/test_ss_bridge_watcher.py`: attributable deck stops, bridge-gap respawn). AWR-192 (menubar overhaul) adds 7 cases to `tests/test_bridge_menubar.py` (91 total there; each base-class case runs twice via the M2 `NativeInstallGateTests` subclass): `test_led_row_fields_truth_table` + `test_led_row_fields_malformed_never_raises` (the pure LEDs glance-row helper: on/off/unknown states, fps only while realtime-active, degraded-reason surfacing, malformed-status fail-soft), `test_compact_status_lines_returns_four_rows_both_branches` (pins the `refresh_` zip contract — a row-count mismatch silently drops rows), `test_menu_blueprint_selector_inventory_exact` (exact selector set, each once), `test_menu_blueprint_has_only_essential_status_and_no_dead_controls` (Status submenu holds exactly four `status_rows`; old top-level `status_item` and removed everyday controls stay absent), `test_menu_blueprint_maintenance_block_order`, and `test_menu_blueprint_attrs_unique`. AWR-221 adds full-lifecycle `_spawn_watched` busy/restore/alert paths, quit confirm, and target-patch completion verification (`has_get_task_allow` on the watcher thread → native NSAlert). RB7216 restores the common **Patch Rekordbox** action (both editions) with pure `patch_rb_button_text`/`patch_rb_button_enabled` state table, a distinct `_patch_rb_in_progress` busy flag so refresh cannot overwrite **Patching Rekordbox…**, source/frozen argv pins, and completion resets that prove **Rekordbox Patched** only after a positive verified verdict; the old standing status row stays absent (Status remains four rows). The Rebuild-USB state-aware item (usb_rebuild_button_state_spec_2026_07_12) adds 9 cases: `test_usb_button_text_table` + `test_usb_button_enabled_table` (busy label wins every state; actionable only source+outdated), `test_classify_usb_state_table` (no_usb/up_to_date/outdated decision, every uncertain input → outdated), `test_usb_source_signature_tracks_changes` (stat-signature stability, mtime/add/remove sensitivity, absent-root stability), `test_usb_source_roots_pin_build_inputs` (drift pin against `make_stick.sh` staged inputs), `test_usb_stamp_round_trip`, `test_read_stick_generation`, `test_usb_rebuild_completion_resets_flag_and_stamps_only_on_success` (success/failure/signal-death all reset the busy flag; stamp on exit 0 only), `test_record_usb_rebuild_stamp_uses_click_time_fingerprint` (stamps the click-time fingerprint, never a post-build recompute; no pending fingerprint → no stamp) | needed before command changes; menubar/watcher tests do not prove live watcher or bridge process health; blueprint tests pin layout data, not rendered NSMenu behavior; Patch Rekordbox proves target entitlement UI only (no live codesign/apply/AppKit/hardware); AWR-222 stock foreign-Mac attach remains live-unvalidated / unknown |
...
| Frozen-app foreign-Mac fixes | `tests/test_bridge_menubar.py` adds `FrozenDefectHelperTests`: the flock singleton `acquire_menubar_lock` refuses a second holder and re-acquires after release (DEFECT 2, replaces the pgrep/`MENUBAR_PATTERN` guard a frozen argv never matched), `export_button_enabled` is false when frozen (DEFECT 4 authoring-only), `laser_pad_argv`/`led_pad_argv` resolve source vs frozen (`--run-laser-pad`/`--run-led-pad`), `_format_child_failure` names the exit code + stderr tail (DEFECT 6). The pad handlers now `open_pad` (probe port → spawn if down → open) instead of opening a dead URL (DEFECT 5). `tests/test_rekordbox_patch.py` covers `sanitized_system_env` (DEFECT 3), AWR-223 entitlement/backup guards, the RB7216 root-only signer + one-admin restore markers (mocked `run_shell_via_admin` only — exactly one root-bundle argv, no `--deep`, no nested walk/re-sign, in-script restore markers with OK/FAIL/missing distinction, no second `_restore_app` on admin sign-fail), the native frozen confirmation path (explicit **Apply Patch** only; alert failure fails closed), and App Management hint seams (frozen vs source wording only on `Operation not permitted` under `/Applications`; unrelated sign failures stay silent). `tests/test_usb_launcher.py` pins `NSAppBundlesUsageDescription` in `packaging/rbss_launcher.spec` and adds `--run-laser-pad`/`--run-led-pad` dispatch | pure/flock/temp-dir + monkeypatched-dispatch only; they prove the guard, gate, argv, env-sanitizer, pad-spawn, patch-planner, native-alert intent, and App Management message logic. They do not prove the real frozen build, rendered native alerts, live codesign apply, App Management TCC grant covering the remaining admin `osascript` escalation, shell full-payload round-trip, or live reads. AWR-222 stock attach after clean GTA remains live-unvalidated / unknown; this row is component coverage, not a claim that stock attach is proven impossible or already proven working. |
