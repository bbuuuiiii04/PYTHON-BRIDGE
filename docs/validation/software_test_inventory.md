...
validation_scope: software-validated only; Rekordbox 7.2.16 direct-reader offsets, RB7216 target-patch/menu/USB-builder hardening including the immediate grey initial-check repaint, AppleDouble pruning and final mounted-DMG package validation, native installer guards, AWR-207 split-local-UUID classification, and AWR-236 Venue Check (fake-probe report + menubar alert/first-run marker pins; no network/MIDI/serial in tests) software-tested; physical foreign-Mac patch/attach/install and lighting hardware unvalidated
---
...
| USB foreign-Mac live-reader authorization (AWR-222) | Dormant AX MEASUREMENT probe: `tests/test_usb_launcher_ax_probe.py` (fake facades only — no ApplicationServices/TCC/Rekordbox/lsof/App Support/`/Volumes` execution); dispatch pins in `tests/test_usb_launcher.py`; lock/spec pins in `tests/test_make_stick.py`. Current-code audit neighbors remain `packaging/sign.sh`, `rekordbox_patch.py`, `rb_memory.py`. | **Probe implemented/software-tested/not executed.** Tests prove dispatch isolation, preflight refusals, permission prompt gating, BFS caps, sanitizer canaries, private `0600`/`0700` modes, token stability, sidecar abstention, process-wide `lsof` non-assignment, cadence unmeasured-without-reference, and static forbidden-symbol guards. No live AX evidence, no TCC evidence, no USB read, no foreign-Mac parity claim. Target `get-task-allow` is the expected TimecodeLink-style mechanism; positive entitlement proves the Rekordbox patch only. Stock Apple-Silicon attach after patch+verify+GTA+relaunch is live-unvalidated / unknown (not a confirmed caller-authorization blocker). The dormant AX measurement probe itself adds no menu item and no runtime wiring; the separate RB7216 **Patch Rekordbox** action targets entitlement only. AX is not a selected replacement reader. Not a reader until the operator stock-SIP live gate (+ optional E3 matrix) produces evidence. |
| Runtime commands | parser/handler/status writer tests plus menubar watcher-launch tests and watcher function tests. AWR-192 pins the slim menu inventory and four-row Status submenu. `tests/test_bridge_menubar.py` also pins the LEDs Status-row display-only path: realtime `last_error` (e.g. `transport_send_failed`) surfaces as degraded suffix `sends failing` under **On**, healthy empty `last_error` stays clean, and a cloud-adapter `degraded_reason` still wins over realtime. Frozen toggle tests pin `close_monitor()` once on owned-child stop and once on adopted-lockfile-pid stop, and not on the start/spawn branch. AWR-221/RB7216 pins the common **Patch Rekordbox** action, distinct checking/patching flags, source/frozen argv, cancel suppression, and watcher-thread GTA + deep/strict completion verification; **Checking Rekordbox…** and **Rekordbox Patched** are grey, while invalid/absent/unknown stays actionable. Rebuild-USB tests pin state text, click-time fingerprinting, complete build-input roots, and success-only stamping. AWR-236 pins **Venue Check…** under Tools in both editions (pads stay source-only), worker-thread dispatch, alert assembly with optional **Patch Rekordbox…** routing to the existing patch action, and first-run `venue_check_seen` marker helpers; `tests/test_venue_check.py` covers pass/warn/fail/info permutations with injectable fake probes only (no network/MIDI/serial). | Menubar tests prove pure state, dispatch, and completion logic only. They do not prove rendered AppKit UI, live codesign/App Management, bridge/watcher health, a physical foreign-Mac attach, Local Network/IAC grants, or hardware. AWR-222 remains live-unvalidated / unknown; AWR-236 guest-Mac live Venue Check run pending. |
| USB launcher (M2 native install/PURGE, AWR-186) | `tests/test_make_stick.py` drives `packaging/make_stick.sh` through its documented test seams (no PyInstaller/hdiutil): staging layout (app + `RBSS_payload/spectral_cache` + `RBSS_payload/home/` five-file parity list), absent-source skip-with-note, existing-but-unreadable ABORT naming the step, a malformed/unreadable `soundswitch_pack_player.json` OR a declared non-empty `pack_path` that is not a readable directory ABORT nonzero (a stick never ships claiming success with no show), an absent config or empty `pack_path` staying backward-compatible no-pack, PIONEER-volume target refusal, `bash -n`. `tests/test_enttec_dmx_pro.py` proves `find_enttec_port` returns a device ONLY on positive ENTTEC identity (manufacturer/product/description strings): a lone generic FTDI (VID 0x0403) and a bare `usbserial` device name are rejected, two identified Enttecs are ambiguous (None), detection is metadata-only (no port opened), and `resolve_enttec_port` fails closed to the operator's configured port when no positive identity is present. `tests/test_install_controller.py` covers the native installer/purge pure seams: DMG/translocation detection + `bundle_root` walk + install-offer matrix + payload-sibling discovery; `perform_install` full layout with interim-format manifest exactness (absolute paths, app first, appended per success) and partial-failure step naming with the manifest listing only what landed; `launch_profile.app_support_config_env` present/absent env maps (all four seams incl. `RBSS_LASER_COLOR_MAP_CONFIG`); `perform_purge` allowlist + `..` rejection + three-root removal order + own-app deferral + failure-records-and-continues (rmtree fault injection); `launch_profile.resolve_state_path` frozen mapping vs byte-identical source passthrough, wired at both consumers (`led_config` store_path parse, `drop_presentation.LEARNED_STORE_PATH`). `tests/test_bridge_menubar.py` adds the frozen gates (no module-level `install_controller` import; install/purge worker failure marshaling). `tests/test_usb_launcher.py` adds App-Support-copy-wins and explicit-env-wins wiring. `tests/test_laser_color_engine.py` adds the color-map env seam | pure/temp-dir tests only; they prove staging/install/purge decision logic and env wiring, NOT that a rebuilt DMG carries the payload, that the NSAlert flows render, that a running bundle Trash-moves cleanly, or foreign-Mac Gatekeeper/TCC behavior — that is the operator walkthrough (runbook parity table) Also: GENERATION→Info.plist injection seam (`tests/test_make_stick.py`), `operator_install_failure_message` plain-language alert boundary (`tests/test_install_controller.py` + menubar alert pin), and USB ANLZ PPTH leading-slash acceptance (`tests/test_filepath_resolver_sidecar.py`). AWR-237: `tests/test_make_stick.py` pins `packaging/libhidapi_arm64.lock` as a SOURCE tarball SHA-256 + build-recipe lock (not a built-dylib digest), spec resolve to the wheelhouse-built artifact (no Homebrew candidate paths), and make_stick seams `verify_hidapi_source_tarball` / `validate_hidapi_dylib` / `verify_hidapi_lock` / `require_hidapi_in_app` (wrong source hash, missing tarball, minos > 12.3, missing bundled dylib all fail closed; pure/seam only — no real clang in unit tests); `tests/test_usb_launcher.py` pins `prepare_frozen_hidapi` (source no-op; frozen loads bundled path + sets HOMEBREW_PREFIX; missing dylib no-op) and that `_run_streamdeck` calls it before importing StreamDeck. No real HID / guest Stream Deck device claim. |
| Frozen-app foreign-Mac fixes | `tests/test_bridge_menubar.py` adds `FrozenDefectHelperTests`: the flock singleton `acquire_menubar_lock` refuses a second holder and re-acquires after release (DEFECT 2, replaces the pgrep/`MENUBAR_PATTERN` guard a frozen argv never matched), `export_button_enabled` is false when frozen (DEFECT 4 authoring-only), `laser_pad_argv`/`led_pad_argv` resolve source vs frozen (`--run-laser-pad`/`--run-led-pad`), `_format_child_failure` names the exit code + stderr tail (DEFECT 6). The pad handlers now `open_pad` (probe port → spawn if down → open) instead of opening a dead URL (DEFECT 5). `tests/test_rekordbox_patch.py` covers `sanitized_system_env` (DEFECT 3), AWR-223 entitlement guards, the root-only one-command signer with **no** full-app backup, native authorization argv/script seams, the native frozen confirmation path (explicit **Apply Patch** only; alert failure fails closed), and App Management hint seams. `tests/test_usb_launcher.py` pins `NSAppBundlesUsageDescription` in `packaging/rbss_launcher.spec` and adds `--run-laser-pad`/`--run-led-pad` dispatch | Unit tests prove the guard, gate, argv, env-sanitizer, pad-spawn, no-backup failure wording, patch planner, native-alert intent, and App Management message logic. Separate local evidence: a real `/Applications` RB 7.2.16 target patch passed deep+strict + GTA and a signed frozen RBSS probe completed an inert native-authorized write/remove in its bundle. This does not prove a physical friend-Mac patch, rendered dialogs, or live reads. AWR-222 stock attach after clean GTA remains live-unvalidated / unknown. |
| RB7216 patch, native install, and USB publication hardening (2026-07-13) | `tests/test_rekordbox_patch.py`, `tests/test_bridge_menubar.py`, `tests/test_install_controller.py`, `tests/test_make_stick.py`, `tests/test_filepath_resolver_usb_twin.py`, `tests/test_smart_transitions.py` | Proves GTA-only entitlement merge; GTA+deep/strict menu truth; repair of GTA with invalid signature; argv-carried privileged script; root-owned transient entitlement plist; no-follow lock; cancel suppression; an immediate grey initial check on the first paint; safe DMG detach argv; complete USB source roots; `/Volumes` + clean/unchanged-source publication guards; payload-symlink rejection; AppleDouble/`.DS_Store` pruning before the package manifest; mutable transport metadata excluded from outer FAT publication hash authority; required final mounted-DMG deep-signature plus native installer validation before publication; Govee mode `0600`; conservative rollback failure copy; partial-stage cleanup; and split-local-UUID early-worker behavior while `/Volumes` stays device-export. The mounted-DMG gate is also exercised by the real builder run; its unit tests pin ordering and staging behavior without invoking `hdiutil`. These are software tests and local read-only/signature evidence, not a physical foreign-Mac, Gatekeeper/TCC, bridge attach, or hardware-lighting result. |
...
AWR-288 adds `tests/test_section_energy_v0.py` (20 tests) for the energy E2 per-section grade layer
`section_energy_v0.py` (runtime-imported on the ANLZ worker, `RBSS_SECTION_ENERGY` default-off,
STATUS-ONLY — no LED/laser/SS module may read the grades). It proves: within-track gain-invariance
(loudness_ref_db-relative, a uniform per-track master offset cancels); `library_scaled = within_track ×
track_weight` or null; the E1 track-weight store consumed only through `load_track_weight_store`, which
refuses `accepted:false`/missing/malformed/wrong-version stores by construction; absent data (no store /
no v4 / no markers / short section / non-finite) → absent grades, never fabricated; the pinned G1
(coverage 0.95) / G2 (0.90 of tracks at ≥0.10 within-track spread) gate math where a failed gate is a
valid reported result; the flag-off byte-identical kill path (no computation, no `ANLZ_DATA` payload key,
no status key); and the AST static import-fence invariant (only `state_manager.py` + `tools/` + `tests/`
may import). Stdlib-only; duck-typed synthetics, no real cache/DB.

AWR-290 adds `tests/test_drop_energy_v0.py` (14 tests) for the energy E3 per-drop grade layer
`drop_energy_v0.py` (runtime-imported on the ANLZ worker, attached to `DropDecision` at tick-side plan
build, `RBSS_DROP_ENERGY` default-off, STATUS-ONLY — no presentation/laser/LED path may read
`DropDecision.energy_grade`). It proves: `within_track` = mean of three gain-invariant terms (body
loudness-relative, sub-duty vs ref-12, onset-ratio vs the track's own `onset_mh_p90`); `library_scaled =
within_track × track_weight` or null; the E1 store reached only via the E2 loader/memo (no second store
path); the G1 (coverage 0.95) / G2 (IQR ≥0.05) / G3 (drop-vs-low separation ≥0.15) gate math; the
attach-match discipline (every surfaced grade comes from plan-attached true-drop decisions built from
`meta.smart_drops`, no surface enumerates the raw marker set); absent data → absent grades; the flag-off
byte-identical four-surface kill path (no compute, no payload key, no status key, all
`DropDecision.energy_grade` None); and the AST static import-fence invariant (only `state_manager.py` +
`tools/` + `tests/` may import). Stdlib-only; duck-typed synthetics, no real cache/DB.
...
## Pack Startup + Laser MIDI Port Hot-Plug Retry (AWR-238)

- `tests/test_soundswitch_pack_startup.py` (`PackStartupRetrySupervisorTests`):
  injected factory fails N times then succeeds → late-attach publishes a live
  `PackRuntime`, updates cleanup owners, swaps `LaserSceneExecutor` backend;
  happy-path `reason=pack` starts no supervisor; `stop_event` during retry
  exits promptly (no thread leak).
- `tests/test_midi_output.py`: boot `port_unavailable` then injected mido port
  appears → throttled reopen clears degraded and queued sends flow; existing
  `send_error` reopen/cooldown cases remain green.
- `tests/test_soundswitch_midi_input.py`
  (`test_invalid_port_error_mid_run_survives_and_rebinds`): bound worker raises
  injected `InvalidPortError` mid-run → marks `input_port_gone`, same thread
  stays alive, rebinds when the factory succeeds again (no `worker died`).
...
| tests/test_led_color_engine_m2_patch_f.py | Patch F default-bank cleanup + AWR-265 FINAL clone-free roster (no legacy_color_suffix bank; retired color-suffix looks absent; bases in default) | Patch F / AWR-265 |
| tests/test_awr265_color_clone_collapse.py | AWR-265 FINAL color-level A/B: promoted residual pairs resolve under blue_cyan; clones gone; collapse tool honest on clone-free config | AWR-265 |
| tests/test_awr266_sim_lab_parity.py | AWR-266 shared ledsim-player clock + sim presentation default + pad serve tripwires | AWR-266 |
| tests/test_awr277_lab_cue_first_class.py | AWR-277 accepted Template Lab cues first-class in the pad (play-by-name renders `lab_<draft>` frames end-to-end; slot-color inject; "Lab cue" labeling not cloud) + Timing dropdown demoted to a read-only design note (`timing_mode` preserved) | AWR-277 |
| tests/test_awr278_lifecycle_reconcile.py + tests.test_led_ui_integrity.Awr278AuditFixIntegrityTests | AWR-278 post-Accept lifecycle reconciliation + audit fixes: A1 Reject un-wires an untouched accepted look (keeps a pad-edited one), A2 Delete flips the draft to iterating, A4 re-Accept after rename updates in place (no duplicate scene_ref; `rename_look` drops the stale live look), B1 Test-Palette change keeps the live lab tweak + snapshot, B3 cue length clamps to >=1 (server + CueTimer), and static integrity for A3 look-name titles, B2 view-aware `renderBpmScope`, B4 duplicate/rename res.ok, C `.topbar-row[hidden]` guard | AWR-278 |
| tests/test_awr279_breaker_fixes.py + updated tests.test_led_pad_service snapshot-fallback cases | AWR-279 breaker-audit fixes (14 findings): live-safety STOP-drawer stacking/offset + never-stopping "Play once" (`CueTimer` belt + one shared `clamp_cue_beats`); conflict honesty (Reload discards stash, stale play-once aborts, reload-failure UI, ownership-warning lifecycle); input hygiene (single shell Escape owner, 60-char name caps, archive-while-playing guard, Preview-tempo 60–200 clamp); sim message-preference/dialog-keyboard/rail-expand; snapshot-fallback only on real live-tune | AWR-279 |
| tests.test_led_ui_integrity.Awr280PhoneShellTests + tests.test_awr279_breaker_fixes.Awr279StopButtonReachable.test_stop_stays_reachable_at_phone_width + tests/frontend/test_pad_touch.py (playwright) | AWR-280 phone-first pad shell (≤700px, chrome/layout only, desktop >700px byte-identical): the session controls fold into a `<details id="sessionFold">` disclosure (display:contents + hidden summary on desktop, tap-to-open row on the phone), the transport dissolves so the identity row is Pad/Lab/Sim + 📱 + STOP on one line, phrase pills stay one scrollable row, banners truncate with tap-to-expand, "Pad look edits" hides at count 0 and shows the count when nonzero, and the Sim tab shows a phone note instead of a dead localhost hop; playwright layout checks: first look tile within the top 25% at 390×844, STOP ≥44px + hit-testable with the editor drawer open, fold collapsed by default / opens on tap, desktop fold inline (open, summary display:none) | AWR-280 (does not prove on-device iOS Safari behavior or hardware output) |
...
- `tests/test_bridge_log.py` (46 tests) covers `build_record()` field order/optional-key
...
- `tests/test_bridge_view.py` (128 tests) covers the pure viewer layer: `parse_record` (tolerant of
