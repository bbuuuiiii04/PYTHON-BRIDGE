...
| Frontend tools | syntax and smoke tests; AWR-269 `tests/test_led_pad_playback_mirror.py` (tee ring, stalled-SSE cadence, clean shutdown, dry-run ordered RGB match, Lab Watch-live tripwire) | does not prove live safety or room-visible mirror fidelity |
...
| tests/test_led_color_engine_m2_patch_f.py | Patch F default-bank cleanup, legacy_color_suffix storage bank, scene_ref registration, generic drop pairing, no static slot_colors params, solid reachability through default generics; AWR-265 Step 2 roster (clones only in legacy; bases in default) | Patch F / AWR-265 |
| tests/test_awr265_color_clone_collapse.py | AWR-265 Step 0 colorway→palette mapping + seeded frame A/B harness (CONFIG_EDIT / IMPOSSIBLE / OUT_OF_SCOPE) | AWR-265 |
