...
| USB-export local-twin resolution (AWR-207) | `tests/test_filepath_resolver_usb_twin.py` (7 cases), existing beatgrid/hotcue resolver suites, and the local/local-twin ANLZ handoff pins in `tests/test_smart_transitions.py` | Pure comparator/fake DB/event-payload tests prove unique-match and fail-closed behavior without a live process. They do not prove a mounted real USB, room-visible lighting, or AWR-208 foreign-laptop portability. |
