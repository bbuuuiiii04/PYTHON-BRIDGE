...
- Follow the live music's tempo (AWR-275). When the bridge status heartbeat is fresh (the AWR-261
  5-second window) and carries a valid `bpm`, pad/lab playback follows that tempo instead of the
  manual Preview tempo. It rides the existing 0.25s ownership poll (no new loop): every other tick
  (~0.5s, the heartbeat write cadence) `PadPlayback._follow_tempo_from_status()` reads the fresh
  status via `OwnershipGate.read_fresh_status()` and calls `apply_tempo_follow()`. The clock is
  re-anchored at the current beat before its rate changes, so the animation only speeds up or slows
  down — there is no phase jump (`SyntheticClock.set_bpm`). A small live-tap wobble is debounced:
  the clock is only re-anchored when the bpm moves by more than 0.3 BPM. When the bridge is not
  running or its status is stale, playback falls back to the operator's manual Preview tempo exactly
  as before, and the manual `set_bpm` is remembered while following so the fallback is instant. The
  UI shows a read-only "Following your music · N · tempo set by the live mix" chip (`#tempoModeChip`,
  rendered by the shared `PadTempoMode` helper) and disables the manual +/- tempo controls while
  following; the Preview tempo control stays for design-time work when no music is playing. This is
  pad-process only — it touches no bridge runtime files and does not change live automation, which is
  already beat-locked. Beat-position alignment to the deck is deliberately out of scope (the
  heartbeat has no beat position; that would need a separate bridge-side status field).
...
- **Accept wires into the show (AWR-260).** Accept still implies Save (AWR-251), but it also
  ...
  “Live — untagged shelf; tag a phrase to join rotation.”
- **Post-Accept lifecycle reconciliation (AWR-278).** Once a draft is wired into the show, the
  durable link is the wired look's `scene_ref=lab:<draft>` (found by scanning, so it survives a
  look rename — no separate stored back-link). The four lifecycle actions now keep the Lab ledger
  and the live show in sync:
  - **Reject un-wires (A1).** Reject removes the wired look + its bank memberships through the same
    commit + `led_reload_looks` pipeline Accept uses, so “stays out of the show” is true. A look
    the operator changed on the Pad since accepting, or one shared by a drop pair / used as a
    stop-look, is **kept** with an honest message — never silently destroyed. A never-wired draft
    just flips to Rejected as before.
  - **Delete flips the ledger (A2).** Deleting a wired look on the Pad flips its draft
    `accepted → iterating` (best-effort; a ledger hiccup never fails the delete).
  - **Rename is title-safe (A3).** A lab cue's Pad card/editor title follows the LOOK name
    (`human(name)`), not the frozen `scene_ref` draft id; `scene_ref` stays the renderer pointer.
    `rename_look` also marks the old name deleted so the merge-commit drops the stale live look.
  - **Re-Accept updates in place (A4).** Accepting an already-wired draft (even after a rename)
    updates that one look instead of creating a second look with the same `scene_ref`.
