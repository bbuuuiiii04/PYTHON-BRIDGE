...
| `led_clear_blackout` | none | `reason` | Rejects unknown fields. `reason`, when present, must be a non-empty string. | With `reason`: discards exactly that named owner from the blackout-owner set (AWR-154); the strip stays dark while any other owner remains. Bare (no `reason`): operator authority — clears every blackout owner at once and logs one `[RGB] blackout-clear-all` outcome line naming what was cleared (AWR-155). |
| `led_clear_scene_override` | none | none | Rejects all payload fields except `cmd`. | Clears LED scene override callback if wired. |
| `led_reload_looks` | none | none | Rejects all payload fields except `cmd`. | Reloads LED look-director config from disk and re-imports lab effects in the frame-engine child (AWR-260). Runs on the command thread — never on the 200 Hz push loop. Atomic look-table swap between ticks. |
