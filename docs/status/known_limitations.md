...
- The Art-Net truth-check gate is a temporary final-retirement measurement tool,
  not a reusable Art-Net product. It is default-off, observes/compares
  SoundSwitch U0 against bridge U1, and does not prove optical laser output or
  physical fixture behavior. A clean subset is not PASS; missing coverage remains
  INCOMPLETE and any broken evidence chain is INVALID.
...
- Passive SoundSwitch U0 parity evidence now covers scripted witnesses, Autoloop samples, and the Static Look assertion fallback. Fresh software export now has zero active `unverified_parity` documents; this clears the trusted pack publication software gate but still does not prove live sender health or physical fixture output.
