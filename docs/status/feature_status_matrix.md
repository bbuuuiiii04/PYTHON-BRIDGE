...
| SoundSwitch offline decoder/exporter/pack/verifier | implemented | software-tested | pinned SoundSwitch 2.10.3 canonical UUID/RAVE profile only | Strict read-only full rescan, deterministic canonical 95-artifact export, and independent verification. Current proof is 29/0/0; exact inventory is 232 render + 1 catalog-tail cues, 32 Static Looks, 42 autoloops, and 45 scripted records. Export currently requires a new destination; one-click canonical replacement/reload is not implemented. |
| SoundSwitch scripted pack player/runtime/direct-DMX lane | partial | substantial software/wire tests; live runtime/hardware unvalidated | pinned pack/local setup only | Loader/player, MIDI adapter, backend abstraction, Enttec sender, default-off config, startup, StateManager driver, commands, and sanitized basic status exist. All 32 active scripted tracks export/render. Runtime pause-vs-stop, explicit scripted-mode authority, controller-health fail-to-zero, and complete operational status remain open; local pack config is absent. |
...
| SoundSwitch export/operator UI | planned | no implementation | pinned canonical project only | Required `Export from SS` menubar action, one canonical pack replacement, progress/result feedback, and chained reload are absent. SoundSwitch save remains operator-owned. |
