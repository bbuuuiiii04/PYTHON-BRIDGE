...
| SoundSwitch offline project decoder/models | implemented | software-tested | pinned SoundSwitch 2.10.3 canonical UUID/RAVE profile only | Strict read-only source decode with frozen models; current-corpus coverage includes the 232 render-cue plus one catalog-tail split. No project mutation or live bridge integration. |
| SoundSwitch exporter/pack/verifier/player/backend/Enttec | planned | unvalidated | unsupported currently | Production artifacts, config/commands, StateManager/runtime integration, backend selection, Enttec output, and hardware validation are not implemented. |
...
| SoundSwitch catalog/import/UI | partial/planned | decoder software-tested; other pieces unvalidated | pinned decoder only | Task 1 decodes the canonical saved project offline; production exporter/importer/player/UI behavior remains planned and unimplemented. |
