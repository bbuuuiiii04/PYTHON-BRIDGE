...
| AWR-004 | First AGENTS maintainer-preferences dry run | active | Seeded directly in `AGENTS.md`; next real feature change should use one bundled request → agent sorting → implementation → review/repair/handoff, avoid manual branch management, and record missing routes, unclear contracts, token-waste points, or hidden branch/merge-state risks. |
