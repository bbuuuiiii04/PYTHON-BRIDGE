...
| AWR-117 | LED dispatch extraction from `state_manager.py` (audit P5) | `docs/plans/active/led_dispatch_extraction_spec.md` | Planned Codex refactor queue P1-P2: unify the five copy-pasted LED trigger/accept/reject bookkeeping paths behind one helper set, then move the LED dispatch policy into a new `led_dispatch_policy.py` mixin with all `_led_*` fields staying on the `StateManager` instance. Pure refactor — `led_status_provider` payloads, counters, gate reasons, role keys, and log lines must stay byte-identical, proven by the unchanged 2762-test suite. No bridge restart, live process, SoundSwitch, Rekordbox, laser, LED/Govee, MIDI, DMX, Enttec, or hardware action is authorized by the spec alone. |
