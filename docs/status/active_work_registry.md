...
| AWR-107 | SoundSwitch static exporter / bridge-native player | `docs/plans/active/soundswitch_exporter_remaining_work.md` (single current status/roadmap), `docs/plans/active/soundswitch_README.md` (project index), `docs/research/soundswitch/README.md` (RE routing), `docs/architecture/native_autoloop_pack_authority.md` (native Autoloop target behavior), `docs/plans/active/native_autoloop_dmx_runtime_spec.md` (greenlit implementation spec), `docs/plans/active/soundswitch_autoloop_equivalence_oracle_spec.md` (offline equivalence/calibration gate), `docs/plans/active/soundswitch_t7d_capture_evidence_plan.md` (historical T7d evidence/tooling; no longer native gate), `docs/prompts/active/soundswitch_rw7_capture_agent_prompt.md` (legacy T7d prompt; no native gate), `docs/prompts/reviews/soundswitch_rw5_hardware_validation_implementation_review_prompt.md` (reusable review-only handoff), `docs/validation/soundswitch_hardware_validation_procedure.md` and `soundswitch_hardware_runs/TEMPLATE.md` (non-Autoloop operator gate) | Current project proof is 29 PASS / 0 FAIL / 0 INCOMPLETE and 32/32 active scripted tracks are exportable. RW-1 through RW-5, graceful shutdown ownership, the copied menubar status, the connection auto-switch (`set_soundswitch_pack action=enable`, with one bounded `pack_start_failed` retry and no manual pack button), the SoundSwitch-saved Static Override Press/Toggle interaction mode, the repo-local ignored pack path `local/soundswitch/rbss_canonical_pack`, required binding-sidecar stage-before-swap publication, native Autoloop DMX, and the non-Autoloop procedure/template are implemented and software-tested. The latest independent software/wire review found no blocker/high issues; one medium menubar parser mismatch was fixed. Material specs are historical under `docs/plans/completed/soundswitch/`. Native Autoloop DMX uses bridge-owned phase, canonical-pack role-note checks, `AUTOLOOP_TICKS_PER_BEAT = 600`, `phase_offset_beats`, the offline equivalence oracle, and an operator two-flight calibration/A-B run instead of the old six-scenario T7d gate. No post_drop maps are required today: current post-drop falls back to 32-beat drop-bank cycling; mapped post_drop looks must cycle inside the post_drop bank every 32 beats. No operator hardware evidence run exists. Pack blackout routing and the F12 held-blackout hold-expiry fix (edge-case hardening Task 11, now in `docs/plans/completed/soundswitch/`) are deferred pre-go-live: the BLACK OUT pad is currently inert in pack mode (never opened), so there is no live exposure today; decide routing and apply F12 together before any hardware go-live. No live config, restart, runtime command, device, output, or hardware action is authorized unless a task explicitly grants it. SOFTWARE/WIRE-VALIDATED ONLY / HARDWARE-UNVALIDATED. |
| AWR-112 | SoundSwitch DMX cue parity | `docs/plans/active/soundswitch_dmx_cue_mismatch_spec.md`, `docs/plans/active/soundswitch_pack_render_defect.md` | ROOT CAUSE UPDATED. The scripted playback-mixer theory is rejected by operator ground truth and current evidence. The confirmed issue is generated pack content for scripted track `dd42028c-0823-4a8d-ad7e-b26e24180272`: the bridge rendered its pack exactly, but the pack resolved wrong cue content for 14/91 boundaries versus clean SoundSwitch U0. The ignored local canonical pack is patched with capture-derived `oracle_rendered` boundary frames; tracked code now lets scripted playback use verified boundary frames while normal generated packs still reject accidental boundary mutation, and Autoloop rendering remains event-replay based. No bridge restart, runtime toggle, SoundSwitch recapture, process-memory sampling, Enttec/DMX, laser, LED/Govee, Rekordbox, or hardware action was performed or authorized by this patch. SOFTWARE/WIRE-VALIDATED ONLY / HARDWARE-UNVALIDATED. |
