...
| AWR-107 | SoundSwitch static exporter / bridge-native player | `docs/plans/active/soundswitch_exporter_remaining_work.md` (single active checklist/roadmap), `docs/plans/active/soundswitch_README.md` (grouped planning index), `docs/plans/active/soundswitch_rw1a_shutdown_ownership_spec.md` (RW-1A shutdown-ownership spec — implemented + reviewed + revision landed), `docs/plans/active/soundswitch_rw3_mode_authority_spec.md` (RW-3 implementation spec), `docs/plans/active/soundswitch_rw5_operational_status_spec.md` (RW-5 copied operational status), `docs/plans/active/soundswitch_hardware_validation_harness_spec.md` (hardware procedure/template spec), `docs/prompts/reviews/soundswitch_rw3_mode_authority_implementation_review_prompt.md` (RW-3 independent implementation review), `docs/research/soundswitch/README.md` (research authority), `docs/research/soundswitch/soundswitch_importer_exporter_player_codex_spec.md` (original product contract), `docs/plans/active/soundswitch_t7d_capture_evidence_plan.md` (capture blocker plan) | Current project proof is 29 PASS / 0 FAIL / 0 INCOMPLETE; 32/32 active scripted tracks are exportable and the pure scripted renderer plus config/startup/StateManager/backend/Enttec lane exists in software. RW-1 one-click export/replace/reload (incl. change-detection/freshness) is implemented, software-tested, and independently reviewed. RW-1A shutdown-ownership (graceful zero of the live runtime-swapped sender on SIGTERM/SIGINT/atexit) is implemented, reviewed, and the revision landed (`90ba8a2`/`7772bd2`). RW-2/RW-3/RW-4 runtime authority is software-tested. RW-5 adds provider-free copied operational status and a stale-safe combined menubar row; it intentionally does not claim sender or physical darkness. The reviewed hardware procedure/template exist, but no operator evidence run exists. T7d has 2 accepted arm + 2 accepted refire integrity captures, but 4 scenario pairs, identity/holdout reconciliation, and a unique oracle remain; native Autoloop DMX stays software-zero. Current live config/device state was not inspected. No restart/device/output/hardware action is authorized. SOFTWARE/WIRE-VALIDATED ONLY / HARDWARE-UNVALIDATED. |
