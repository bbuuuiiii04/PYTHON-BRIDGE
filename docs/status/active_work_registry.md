...
| AWR-110 | Rekordbox mixer active-deck authority | `docs/architecture/active_deck_authority.md` (operator-authoritative target behavior), `docs/plans/active/rekordbox_mixer_active_deck_re_spec.md` (Ghidra/Ghidra-MCP RE + implementation handoff), `docs/research/rekordbox_mixer_active_deck_re_evidence.md` (current RE proof), `docs/prompts/active/rekordbox_mixer_active_deck_re_continuation_prompt.md` (implementation handoff after RE closure), `docs/prompts/reviews/rekordbox_mixer_active_deck_re_review_prompt.md` (adversarial RE review handoff) | Planned runtime implementation. Current code still treats Rekordbox direct master/playing-only mirror switching as active-deck authority. Target behavior redefines `active_deck` as the playing audible show deck selected by Deck 1/2 upfader and bass EQ, with `rb_master_deck` retained separately for tie/fallback. Static Ghidra/GhidraMCP evidence plus operator-approved passive process-memory proof confirms the local Rekordbox 7.2.11 Deck 1/2 upfader chains, LOW/BASS EQ chains, Deck 1/2 channel ownership, EQ band 2 = LOW/BASS, Deck 1 mid-fader, CFX FILTER param0/param1 chains, relaunch reacquire, and mixer-chain readability after operator-labeled master-button actions. Direct-master byte authority remains current bridge code truth; the mixer JSONL artifacts are not the proof for that byte. Required implementation guardrails call out named mixer offset parser fields/tests, a mixer-specific finite-f32 reader because the existing live-BPM float helper rejects valid mixer values, FILTER tracking as non-authority only, and suppressing both playing-only mirror switching and resume-time direct `active_deck` correction while mixer authority is valid. No local 7.2.11 Deck 1/2 pointer/value-mapping unknown remains for upfader, LOW/BASS EQ, CFX FILTER, Deck 1 midpoint, relaunch reacquire, or mixer-chain readability after operator-labeled master-button actions. Runtime implementation, fail-closed validity/freshness, resolver thresholds/hysteresis, versions beyond 7.2.11, actual play/stop survival with loaded tracks, and hardware-visible behavior remain unimplemented/unvalidated. No bridge restart, additional live capture, process-memory sampling, or hardware action is authorized by the spec alone. |
