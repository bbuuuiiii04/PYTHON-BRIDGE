...
| AWR-107 | SoundSwitch static exporter / bridge-native player | `docs/research/soundswitch/README.md` (directory authority), `docs/research/soundswitch/soundswitch_re_closure_report.md` (RE verdict), `docs/research/soundswitch/soundswitch_importer_exporter_player_codex_spec.md` (implementation spec) | **RE COMPLETE: READY FOR PERFECT EXPORTER SPEC** for SoundSwitch 2.10.3/current RAVE profile. Implementation is not started. It must include complete rescans, 19 IAC Autoloop bindings, 32 active scripts, 232 Attribute Cues, all 32 Static Looks, four DDJ overrides, learned-map updates, and default-off/shadow/hardware gates. No runtime, restart, MIDI/DMX, Enttec, or hardware action is authorized by the research result; software/wire-validated only, hardware-unvalidated. |
