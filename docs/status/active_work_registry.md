...
| AWR-107 | SoundSwitch static exporter / bridge-native player | `docs/plans/active/soundswitch_exporter_remaining_work.md` (single active checklist/roadmap), `docs/plans/active/soundswitch_README.md` (grouped planning index), `docs/research/soundswitch/README.md` (research authority), `docs/research/soundswitch/soundswitch_importer_exporter_player_codex_spec.md` (original product contract), `docs/plans/active/soundswitch_t7d_capture_evidence_plan.md` (capture blocker plan) | Current project proof is 29 PASS / 0 FAIL / 0 INCOMPLETE; 32/32 active scripted tracks are exportable and the pure scripted renderer plus config/startup/StateManager/backend/Enttec lane exists in software. Confirmed remaining work is broader than T7d: menubar one-click canonical replacement/reload is absent; scripted runtime pause-vs-stop, explicit mode authority, controller-health fail-to-zero, and operational status need closure. T7d has 2 accepted arm + 2 accepted refire integrity captures, but 4 scenario pairs, identity/holdout reconciliation, and a unique oracle remain; native Autoloop DMX stays safe-zero. Local pack config and hardware validation are absent. No restart/device/output/hardware action is authorized. SOFTWARE/WIRE-VALIDATED ONLY / HARDWARE-UNVALIDATED. |
