...
| AWR-004 | First main-first bundled workflow dry run | active | Seeded with `docs/agents/operator_workflow.md`; next real feature change should use one bundled request → agent sorting → implementation → review/repair/handoff, avoid manual branch management, and record missing routes, unclear contracts, token-waste points, or hidden branch/merge-state risks. |
