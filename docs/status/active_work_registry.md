...
| AWR-112 | SoundSwitch DMX cue parity | `docs/plans/active/soundswitch_dmx_cue_mismatch_spec.md` | ROOT CAUSE IDENTIFIED / SPEC ONLY. Scripted U1-vs-U0 mismatches are bridge runtime playback-mixer parity gaps: the bridge renders one active deck while SoundSwitch U0 is produced by SoundSwitch playback mixer state, including two-playback crossfade and effective-playback rules confirmed with GhidraMCP. Autoloop bridge-dark/SS-lit mismatches are a separate latched-scene handoff bug with a current software patch/test in the worktree. Runtime fixture-group filtering had a real player leak and is fixed in the current worktree with scripted/static regressions, but that was not the concrete scripted FB4/528e mismatch cause. No live bridge restart, runtime toggle, SoundSwitch capture, fixture pack/config mutation, process-memory sampling, Enttec/DMX, laser, LED/Govee, Rekordbox, or hardware action is authorized by this spec. SOFTWARE-ANALYZED ONLY / HARDWARE-UNVALIDATED. |
