...
| AWR-110 | Rekordbox mixer active-deck authority | `docs/architecture/active_deck_authority.md` (operator-authoritative target behavior), `docs/plans/active/rekordbox_mixer_active_deck_re_spec.md` (Ghidra/Ghidra-MCP RE + implementation handoff), `docs/prompts/reviews/rekordbox_mixer_active_deck_re_review_prompt.md` (adversarial RE review handoff) | Planned. Current code still treats Rekordbox master/playing-only mirror switching as active-deck authority. Target behavior redefines `active_deck` as the playing audible show deck selected by Deck 1/2 upfader and bass EQ, with `rb_master_deck` retained separately for tie/fallback. Static Ghidra decompilation identifies candidate mixer UI value flow and `DjMixerUnit` per-channel fader/EQ state offsets, but Deck 1/2 ownership, EQ band order, stable pointer chain, live values, and operator-approved passive validation remain unproven before implementation. GhidraMCP was unavailable in the planning pass. No bridge restart, live capture, process-memory sampling, or hardware action is authorized by the spec alone. |
