...
| AWR-119 | Stream Deck palette control + drop presentation policy | `docs/plans/active/streamdeck_palette_control_design_spec.md` (expanded design), `docs/plans/active/streamdeck_palette_control_impl_spec.md` (Package 2 implementation spec), `docs/architecture/palette_control_authority.md`, `docs/architecture/drop_presentation_authority.md` (operator-authoritative behavior contracts) | Package 1 blackout re-wire is implemented/software-tested under AWR-111. Package 2 Stream Deck palette control is in implementation under the `streamdeck_palette` change contract: palette queue/override-fade/lock, `white_sand`, LED/Laser mute pads, Rainbow mode, feedback file, and pinned pad iconography are software-targeted only until tests land. Laser Solo pad + zero-RNG auto-solo tiers (hotcue/learned/gear-shift/record-breaker), finale guarantee, and track personality remain PLANNED / DESIGN-INTENT for later packages. No bridge restart, live process, SoundSwitch, Rekordbox, laser, LED/Govee, MIDI, DMX, Enttec, or hardware action is authorized by the spec alone. SOFTWARE-VALIDATED ONLY / HARDWARE-UNVALIDATED once software tests pass. |
