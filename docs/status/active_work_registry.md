...
| AWR-112 | Graphify repo-map integration | `docs/prompts/active/graphify_install_prompt.md`, `docs/setup/graphify.md` | Local Graphify CLI installed as `graphifyy` 0.9.2 via `pipx`; repo graph is code-only (`258` code/config files, `0` docs/images/papers) with `graphify-out/` gitignored. Hook decision: manual query for Claude and Codex; no PreToolUse/read-interception hooks and no post-commit graph hook. Pilot kept it as an orient-before-read aid: it found the right source files for broad architecture questions, but exact-symbol blast-radius lookups can still be cheaper with `rg`. No bridge restart, live process, config, device, output, or hardware action was authorized or performed. |
