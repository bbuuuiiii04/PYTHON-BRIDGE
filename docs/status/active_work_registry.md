...
| AWR-112 | Graphify repo-map integration | `docs/prompts/active/graphify_install_prompt.md` | Install + tune Graphify (local code-AST relationship map) for orient-before-read. Locked: manual query (no always-on hooks), code-only (no cloud key), `graphify-out/` gitignored, flat module layout preserved. Pilot gate (3 known-answer questions) decides keep/drop. Not started. |
