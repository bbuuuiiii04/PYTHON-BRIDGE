...
| AWR-111 | Laser color (CH8/CH9 color control) + blackout re-wire | `docs/plans/active/laser_color_engine_design_spec.md` (expanded design), `docs/architecture/laser_color_authority.md`, `docs/architecture/laser_blackout_authority.md` (operator-authoritative behavior contracts) | PLANNED / DESIGN-INTENT — **not implemented**. Fable review+expand pass 2026-07-04 with operator answers: bridge-owns-frame architecture, fixed-color quantizer model, white-moment mirroring, post-drop settle, blackout owner re-wire. Only the mapper's CH8/CH9 value table is blocked on operator inputs (fixture profile + live visual validation); all plumbing is implementable against a pass-through table. Verify against code before any implementation. |
| AWR-119 | Stream Deck palette control + drop presentation policy | `docs/plans/active/streamdeck_palette_control_design_spec.md` (expanded design), `docs/architecture/palette_control_authority.md`, `docs/architecture/drop_presentation_authority.md` (operator-authoritative behavior contracts) | PLANNED / DESIGN-INTENT — **not implemented**; design approved by operator 2026-07-04 (gate 2). Palette queue/override-fade/lock, `white_sand`, LED/Laser mute pads, Laser Solo pad + zero-RNG auto-solo tiers (hotcue/learned/gear-shift/record-breaker), finale guarantee, track personality, Rainbow mode, feedback file + pad iconography. Codex implementation specs AUTHORED 2026-07-04 (Packages 1-4: `laser_blackout_rewire_spec.md`, `streamdeck_palette_control_impl_spec.md`, `drop_presentation_impl_spec.md`, `laser_color_impl_spec.md`); handoff awaiting operator go. No implementation, bridge restart, or hardware action authorized yet. |
