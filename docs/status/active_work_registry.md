...
| AWR-111 | Laser color (CH8/CH9 color control) + blackout re-wire | `docs/plans/active/laser_color_engine_design_spec.md` (expanded design), `docs/architecture/laser_color_authority.md`, `docs/architecture/laser_blackout_authority.md` (operator-authoritative behavior contracts) | Blackout re-wire Package 1 is implemented/software-tested in current worktree: smart-side blackout latches survive pack-backend note rejection and feed the single StateManager pack mask writer alongside manual MIDI-input blackout. Laser color Package 4 plumbing is implemented/software-tested with a disabled/all-null `config/laser_color_map.json`, so shipped output remains byte-identical authored pass-through until the operator CH8/CH9 chart lands and `enabled` is explicitly set true. The chart remains operator-gated (fixture profile + live visual validation), Package 3/drop-presentation policy is separate future work, and all hardware-visible behavior remains unvalidated. |
| AWR-119 | Stream Deck palette control + drop presentation policy | `docs/plans/active/streamdeck_palette_control_design_spec.md` (expanded design), `docs/plans/active/streamdeck_palette_control_impl_spec.md` (Package 2 implementation spec), `docs/architecture/palette_control_authority.md`, `docs/architecture/drop_presentation_authority.md` (operator-authoritative behavior contracts) | Package 1 blackout re-wire is implemented/software-tested under AWR-111. Package 2 Stream Deck palette control is implemented/software-tested under the `streamdeck_palette` change contract: palette queue/override-fade/lock, `white_sand`, LED/Laser mute pads, Rainbow mode, feedback file, pinned pad iconography, runtime commands, and config/example wiring. Laser Solo pad + zero-RNG auto-solo tiers (hotcue/learned/gear-shift/record-breaker), finale guarantee, and track personality remain PLANNED / DESIGN-INTENT for later packages. No bridge restart, live process, SoundSwitch, Rekordbox, laser, LED/Govee, MIDI, DMX, Enttec, or hardware action is authorized by the spec alone. SOFTWARE-VALIDATED ONLY / HARDWARE-UNVALIDATED. |
