...
| Rekordbox mixer active-deck authority | static RE candidate evidence only | unvalidated | Target behavior/spec exist only. Static Ghidra decompilation identified candidate UI value flow and `DjMixerUnit` fader/EQ state fields, but Deck 1/2 ownership, low/mid/high band order, stable pointer chain, live values, and decoded physical labels still require review plus operator-approved passive proof before runtime authority can be trusted. |
