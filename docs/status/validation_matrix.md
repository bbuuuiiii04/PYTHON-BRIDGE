...
| LED/Govee cloud | partial | unvalidated | Cloud path exists; device behavior must be logged. The pure lifecycle resolver does not replace live LED dispatch; `led_dispatch_policy.py` mirrors the same real-crossing drop-impact gate in the StateManager-owned LED path. Intra-section look rotation is software-tested for long buildup/pre-drop/breakdown/monotonic ambient role keys, stable section/cycle publication, unchanged drop/groove/post_drop keys, and a dispatch-path second look across a buildup cycle boundary. |
