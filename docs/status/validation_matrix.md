...
| LED/Govee cloud | partial | unvalidated | Cloud path exists; device behavior must be logged. The pure lifecycle resolver does not replace live LED dispatch; `led_dispatch_policy.py` mirrors the same real-crossing drop-impact gate in the StateManager-owned LED path. |
