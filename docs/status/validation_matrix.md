...
| Rekordbox mixer active-deck authority | static RE plus operator-approved passive live process-memory proof for local Rekordbox 7.2.11 Deck 1/2 upfader, LOW/BASS EQ, CFX FILTER, Deck 1 mid-fader, relaunch reacquire, and mixer-chain readability after operator-labeled master actions; runtime unimplemented | unvalidated | Target behavior/spec exist, and RE proof now confirms local Deck 1/2 channel ownership, upfader chains, LOW/BASS chains, EQ band 2 = LOW/BASS, CFX FILTER tracking chains, relaunch reacquire, and mixer-chain readability after labeled master-button actions. Direct-master byte authority is existing bridge code behavior, not a field proven by the mixer JSONL artifact. This does not prove SoundSwitch, laser, LED/Govee, DMX, MIDI, Enttec, or bridge-output behavior. Other Rekordbox versions, actual loaded-track play/stop survival, fail-closed validity/freshness, resolver thresholds, status/heartbeat integration, and hardware-visible output remain unimplemented or unproven. |
