...
SoundSwitch pack boundary (reverified 2026-06-29): the deterministic pack, immutable loader/player, MIDI-input adapter, output backends, Enttec sender, default-off config/startup, StateManager scripted driver, native Autoloop resolver, validate-first controls, and copied RW-5 operational status exist as software-tested components. `LaserSceneExecutor` uses one injected backend slot; absent/disabled config preserves existing MIDI, dry-run/none opens neither output, and enabled pack mode owns direct DMX instead of physical MIDI. Laser policy and MIDI execution remain unchanged; the executor now exposes the selected or latched active Autoloop scene so the pack driver can render canonical native Autoloops when SoundSwitch is absent. Sender health is not inferred, and hardware validation remains open. No laser or Enttec hardware validation is implied.
