...
SoundSwitch pack boundary (reverified 2026-06-25 at `bdbf66b`): the deterministic pack, immutable loader/player, MIDI-input adapter, output backends, Enttec sender, default-off config/startup, StateManager scripted driver, validate-first controls, and copied RW-5 operational status exist as software-tested components. `LaserSceneExecutor` uses one injected backend slot; absent/disabled config preserves existing MIDI, dry-run/none opens neither output, and enabled pack mode owns direct DMX instead of physical MIDI. RW-5 changes status publication only; laser decisions and execution are unchanged. Native pack Autoloops remain software-zero pending T7d evidence, sender health is not inferred, and hardware validation remains open. No laser or Enttec hardware validation is implied.
