...
| `docs/research/deck2_reader_diagnosis_2026_07_08.md` | RESEARCH / DIAGNOSIS — read-only root cause of the 2026-07-08 last-song LED blackout, traced past the LED subsystem into the deck-2 rekordbox reader: single working position source (offset chain) with a dead ObjC fallback, and `chain_ok = (snapshot is not None)` treating a frozen field as healthy so it suppresses the fallback (`rb_memory.py:1307/1404`); deck-2-exclusive by construction; 15:13 "laser miss" premise corrected (deck DID flip, laser DID arm — symptom was INVALID churn + position wobble). Three instrumented hypotheses, H2 (wrong field on fresh load) prioritized per operator testimony; analysis only, not implementation-authorizing. |
