...
| `docs/plans/active/docs_orphan_check_spec.md` | PLAN / SPEC (ACTIVE) | Codex spec — add active-doc lifecycle check (orphans + stale-classified) to `tools/check_agent_contracts.py`. Tracked as AWR-007. |
| `docs/prompts/active/graphify_install_prompt.md` | AGENT PROMPT (ACTIVE) | Install + tune Graphify repo-map (manual query / no always-on hooks / code-only). Tracked as AWR-112. |
...
| `docs/prompts/active/rekordbox_mixer_active_deck_re_continuation_prompt.md` | AGENT PROMPT (SUPERSEDED CONTEXT) | Historical implementation prompt for the active-deck authority runtime path. Current code/docs supersede its pre-implementation code-truth section; no live sampling or hardware action is authorized by the prompt. |
