...
| `docs/plans/active/lighting_v1_foundation_fix_spec.md` | PLAN / SPEC (ACTIVE — Codex-ready, gated on operator approval) | Standalone v1-baseline stabilization pass (NOT folded into v2): Task 1 fail-opens the drop-presentation window on every stop (`state_manager.py` `_do_stop`); Task 2 fixes the `lock()` docstring. `drop_presentation` contract; `enabled:false` byte-identity gate held. **Not implemented.** |
