...
| `docs/plans/active/streamdeck_palette_control_design_spec.md` | PLAN / SPEC (PLANNED — design-intent) | Stream Deck pad → LED (and future laser) color-palette control: queue/override/lock gesture, manual-only `white_sand` palette, pad-icon feedback. LED-side design-intent, Fable to expand; wires the M3 live-control surface; laser deferred. **Not implemented.** |
