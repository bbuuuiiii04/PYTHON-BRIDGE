...
# ── Part D.3 — envelope unit tests (TRIGGER semantics, operator re-ruled) ─────
class EnvelopeTests(unittest.TestCase):
    ...
    threshold releases the whole overlay. Re-arm hysteresis stops jitter re-firing."""
    ...
    def test_ccw_and_neutral_are_exactly_neutral(self) -> None:
        ...
        for knob in (0.0, 0.3, 0.49, 0.5):
            ...
            self.assertEqual((st.mix, st.dim, st.armed), (0.0, 1.0, True), msg=f"knob={knob}")
    ...
    def test_jitter_at_exactly_half_is_neutral(self) -> None:
        ...
        self.assertEqual((st.mix, st.dim, st.armed), (0.0, 1.0, True))
    ...
    def test_flood_only_below_threshold_ramps_mix_no_dim_no_fire(self) -> None:
        ...
        # trigger stays ARMED (never crossed). Unchanged from the original sub-threshold law.
        ...
        self.assertTrue(st.armed)         # never fired
    ...
    def test_flood_releases_at_deadband(self) -> None:
        # Returning under the deadband from a below-threshold flood releases (mix -> 0).
        flooded = CfxEnvState(mix=1.0, dim=1.0, armed=True)
    ...
    def test_crossing_fires_drain_once_to_floor(self) -> None:
        ...
        self.assertFalse(st.armed)                              # fired
    ...
    def test_single_tick_jump_floods_and_fires(self) -> None:
        ...
        self.assertTrue(low.armed)
        jumped = cfx_sweep_envelope(0.95, low, 1.0, CFG)
        self.assertFalse(jumped.armed)                            # fired
    ...
    def test_release_after_fire_fades_mix_and_dim_together(self) -> None:
        fired = cfx_sweep_envelope(0.9, CfxEnvState(), 1.0, CFG)  # mix 1, dim floor, armed False
        # knob 0.74 is below thr 0.75 but above thr - hysteresis 0.73: releasing, NOT re-armed.
        ...
        self.assertFalse(half.armed)                             # released but not yet re-armed
    ...
    def test_ride_back_down_through_engaged_band_releases_not_refloods(self) -> None:
        # Operator ruling #3: after firing, riding the knob back toward 12 RESUMES the
        # normal look — it must NOT re-flood the dark hue in the 0.52-0.75 engaged band,
        # even though that band re-arms the trigger for a future push.
        fired = cfx_sweep_envelope(0.9, CfxEnvState(), 1.0, CFG)  # mix 1, dim floor, fired
        self.assertTrue(fired.fired)
        st = fired
        for knob in (0.72, 0.65, 0.58):   # engaged but below thr; walking down
        ...
        self.assertLess(st.mix, fired.mix)            # it released (faded), did not hold at flood

    def test_repush_retriggers_only_after_rearm_hysteresis(self) -> None:
        fired = cfx_sweep_envelope(0.9, CfxEnvState(), 1.0, CFG)  # armed False
        # A dip to 0.74 (above thr - hysteresis 0.73) does NOT re-arm: a re-push doesn't re-fire.
        dip = cfx_sweep_envelope(0.74, fired, 0.05, CFG)
        self.assertFalse(dip.armed)
        repush = cfx_sweep_envelope(0.95, dip, 0.05, CFG)
        self.assertFalse(repush.armed)                           # same firing, no re-trigger
        # A drop to 0.55 (below thr - hysteresis) re-arms, then a push past threshold re-fires.
        rearmed = cfx_sweep_envelope(0.55, fired, 0.05, CFG)
        self.assertTrue(rearmed.armed)
        refired = cfx_sweep_envelope(0.95, rearmed, 1.0, CFG)
        self.assertFalse(refired.armed)                          # fired again
        self.assertAlmostEqual(refired.dim, CFG.dim_floor, places=6)

    def test_threshold_jitter_does_not_refire_while_not_rearmed(self) -> None:
        st = cfx_sweep_envelope(0.9, CfxEnvState(), 0.2, CFG)     # fire, partial drain
        self.assertFalse(st.armed)
        for knob in (0.745, 0.755, 0.745, 0.755, 0.745):         # jitter around thr, never < 0.73
            st = cfx_sweep_envelope(knob, st, 0.01, CFG)
            self.assertFalse(st.armed, msg=f"knob={knob} re-armed unexpectedly")
        self.assertLess(st.dim, 1.0)   # dim never snapped back to 1.0 and re-drained (no machine-gun)

    def test_dt_zero_holds_and_is_safe(self) -> None:
        st = cfx_sweep_envelope(0.9, CfxEnvState(mix=0.3, dim=0.6, armed=True), 0.0, CFG)
        self.assertEqual(st.mix, 0.3)   # no ramp with dt=0
        self.assertEqual(st.dim, 0.6)   # no ramp with dt=0
        self.assertFalse(st.armed)      # fire is edge-triggered, not dt-gated
    ...
    def test_out_of_range_and_nan_knob_are_safe(self) -> None:
        for knob in (-5.0, 5.0, float("nan"), float("inf")):
            st = cfx_sweep_envelope(knob, CfxEnvState(mix=0.5, dim=0.5, armed=True), 0.1, CFG)
...
class DispatchGatingTests(unittest.TestCase):
    ...
    def test_smart_drop_tactical_blackout_is_inert_and_resets_state(self) -> None:
        ...
        # envelope state (armed/mix/dim), or the dark-hue flood re-lights F2's
        ...
        ns._led_cfx_state = CfxEnvState(mix=0.7, dim=0.3, armed=False, fired=True)  # mid-drain
        self.assertIsNone(_compute(ns, 1, now))
        self.assertEqual(ns._led_cfx_state, CfxEnvState())   # neutral: mix 0, dim 1, armed True
...
class ConfigLoaderTests(unittest.TestCase):
    ...
    def test_new_field_defaults(self) -> None:
        cfg = CfxSweepConfig()
        self.assertEqual(cfg.drain_ms, 800.0)
        self.assertEqual(cfg.rearm_hysteresis, 0.02)

    def test_valid_new_fields_load(self) -> None:
        cfg = load_cfx_sweep_config(
            self._write({"enabled": True, "drain_ms": 500.0, "rearm_hysteresis": 0.05})
        )
        self.assertTrue(cfg.enabled)
        self.assertEqual(cfg.drain_ms, 500.0)
        self.assertEqual(cfg.rearm_hysteresis, 0.05)

    def test_new_field_out_of_range_disabled(self) -> None:
        for bad in ({"drain_ms": 0.0}, {"drain_ms": -1.0},
                    {"rearm_hysteresis": 0.2}, {"rearm_hysteresis": -0.01}):
            block = {"enabled": True, **bad}
            self.assertFalse(load_cfx_sweep_config(self._write(block)).enabled, msg=str(bad))
