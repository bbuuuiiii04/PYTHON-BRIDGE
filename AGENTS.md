...
| Phrasing / autoloop / beat / spectral analysis | `smart_phrasing.py`, `smart_rearm.py`, `autoloop_controller.py`, `drop_lifecycle.py`, `beat_math.py`, `audio_spectral_features.py`, `spectral_cache.py` (EVICTFIX 2026-07-24: eviction runs under the same condition that WRITES entries — gated v3 spectral path OR LED identity-v2, not the never-set `RBSS_SPECTRAL_ENABLE`; it skips macOS `._*` AppleDouble files and KEEPS entries whose audio sits on an unmounted `/Volumes` root, so the USB pre-warm survives an unplugged stick), `spectral_profile.py`, `section_energy_v0.py` (AWR-288/291 energy E2 per-section grades: MEDIAN full_db mapped through the −12/0 dB span, saturation/rankability/separation gates; §2/§3 facets published on every grade — `segmentation_basis` {markers,section_map}, `contrast_class` {flat,normal}, and `slope` = RAW `t_pos` dB per ~3 s attentional window with the beat clock derived in-module from v4, all RECORDED-not-surfaced via the `current_section` projection to the frozen 5-key status set; runtime-imported by state_manager on the ANLZ worker, status-only, `RBSS_SECTION_ENERGY` default-off), `drop_energy_v0.py` (AWR-290/291 energy E3 per-drop grades: FOUR required terms — rank-based body + activity (R4 fluxsum swap, replaced onset) + perc_high + growl, seven gates incl. the G7 grade-space separation; §2 facets — the four term VALUES published on every grade dict beside within_track/library_scaled/coverage/body_basis, recorded-not-surfaced; runtime-imported by state_manager on the ANLZ worker, attached to DropDecision at tick-side plan build, status-only, `RBSS_DROP_ENERGY` default-off) | spectral v4: `docs/research/spectral_audio_analysis_redesign.md` |
...
| Offline analysis tooling (no runtime importers) | `energy_model.py`, `tools/analyze_anlz_energy_corpus.py`, `tools/spectral_sweep.py`, `tools/spectral_stick_sweep.py`, `tools/spectral_calibration_report.py`, `tools/spectral_ear_benchmark.py` (AWR-200 Stage-1 ear benchmark + AWR-205 offline gold-label intake: `--emit-gold-template` / `--gold`, read-only), `hardness_v0.py` + `tools/hardness_ablation.py` (AWR-203 offline intrinsic-hardness shadow + read-only ablation; zero runtime importers), `approach_features_v0.py` (AWR-204 offline raw four-view approach descriptors; zero runtime importers, no tool), `track_weight_v0.py` + `tools/track_weight_report.py` (AWR-286/291 energy E1 offline library track-weight descriptor + read-only corpus report/sidecar-store tool; v2 component set body_duty/brightness_med/onset_mh_mean/growl_flatness_mean, TWO GATING acceptance controls — loudness |rho|≤0.50 and worst component-pair |rho|≤0.60 — plus ONE PRINTED DIAGNOSTIC, the dynamic-range correlation reported against the 0.55 reference in `DRAMA_ACCEPT_MAX` and gating nothing since the AWR-291 §5 E1SCRAMBLE demotion (its replacement is the watchdog battery on the next row); schema_version 2 store carries drop_body_distribution; zero runtime importers), `tools/section_energy_report.py` (AWR-288/291 energy E2 read-only corpus saturation/rankability/separation-gate report + boundary-jitter diagnostic; writes no store — the E2 descriptor `section_energy_v0.py` is runtime-imported, listed on the phrasing/spectral row above), `tools/drop_energy_report.py` (AWR-290/291 energy E3 read-only corpus seven-gate report incl. G7 grade-space separation + corpus-basis/attach-match-rate lines; writes no store — the E3 descriptor `drop_energy_v0.py` is runtime-imported, on the phrasing/spectral row), `tools/energy_perturbation_check.py` (AWR-291 offline gain-invariance check by RE-EXTRACTION at two gains — Sturm horse method; temp-dir writes only, NEVER writes the v4 cache or the library; zero runtime importers), `tools/energy_scramble_watchdog.py` (AWR-291 §5 energy E1 spectral-tilt/harness watchdog battery — the REPLACEMENT half of the drama-gate demotion, invoked when an E1 formulation changes, not per corpus run: four E1SCRAMBLE probe classes on the FROZEN 100-track/66-cell/seed-20260724 panel read from `local/e1_scramble_2026_07_24/data/panel.json` (never re-drawn), gating only `c0b_invert` (exact null, rho=1.0000/disp 0) and `c1a_gain` (rho>=0.999, a no-margin harness-reproduction floor) while the two tilt probes are INFORMATIONAL comparative-only; prints the zero-added-gates trade sentence + the panel n verbatim in its header; probe ops carried AST-identical from the E1SCRAMBLE chain prototype, checked by `--verify-ops`; temp-dir writes only, NEVER `put_cached_v4` or the library; zero runtime importers) | `docs/research/anlz_energy_project.md`, `docs/research/spectral_audio_analysis_redesign.md`, `docs/research/spectral_calibration_expansion_2026_07_08.md`, `docs/plans/active/spectral_ear_benchmark_spec_2026_07_10.md` |
...
- Spectral-cache eviction never deletes what it cannot identify: macOS `._*` AppleDouble files are skipped (never read, never deleted), and an entry whose audio lives on an unmounted `/Volumes` root is **unknown, not stale** — deleting those would wipe the USB pre-warm whenever the stick is unplugged.
